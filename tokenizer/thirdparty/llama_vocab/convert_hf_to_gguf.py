...
class ModelBase:
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, *, is_big_endian: bool = False,
        ...
        # changed by randall.zhuo, fix Janus_Pro generate tokenizer fail, 20251129
        try:
            self.model_tensors = self.index_tensors(remote_hf_model_id=remote_hf_model_id)
        except:
            logger.info(f"index_tensors failed, ignore")
    ...
    def load_hparams(dir_model: Path, is_mistral_format: bool):
        ...
        if architectures == ['MultiModalityCausalLM'] and "language_config" in config and isinstance(config["language_config"], dict):
            config = {**config, **config["language_config"]}
...
@ModelBase.register("Qwen2Model", "Qwen2ForCausalLM", "Qwen2AudioForConditionalGeneration", "LlavaQwen2ForCausalLM")
