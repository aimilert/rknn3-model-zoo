...
int main(int argc, char **argv)
{
    ...
    char *endptr = NULL;
    errno = 0;
    uint32_t core_mask = strtoul(argv[5], &endptr, 16);
    if (errno != 0 || *endptr != '\0')
    {
    ...
    // RKNN Context
    rknn_qwen3_5_llm_context rknn_app_ctx;
    memset(&rknn_app_ctx, 0, sizeof(rknn_qwen3_5_llm_context));
    ...
    printf("--> init qwen3_5 llm model\n");
    ret = init_qwen3_5_llm(&rknn_app_ctx, model_path, weight_path, &params, n_params, callback, core_mask);
    if (ret != 0)
    {
        printf("init_qwen3_5_llm fail! ret=%d model_path=%s weight_path=%s\n", ret, model_path, weight_path);
        infer_ret = ret;
        goto out;
    }
    ...
    printf("--> inference qwen3_5 llm model\n");
    first_token = getCurrentTimeUs();
    infer_ret = inference_qwen3_5_llm(&rknn_app_ctx, tensor, n_inputs, &perf);
    if (infer_ret != 0)
    {
        printf("inference qwen3_5 llm fail! ret=%d\n", infer_ret);
        goto out;
    }
...
out:
    ret = release_qwen3_5_llm(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release qwen3_5 llm fail! ret=%d\n", ret);
    }
