...
class PaddleVLVisionMLPAR(torch.nn.Module):
    def __init__(self, visual, mlp_AR):
    ...
    def forward(self, *args, **kwargs):
        vision_outputs = self.visual(*args, **kwargs)
...
parser.add_argument('--export_datapath', type=str, default='../../../../datasets/OmniDocBench_ROI/vision/model_inputs.json', help='data path', required=False)
args = parser.parse_args()

## 加载模型
model = PaddleOCRVLForConditionalGeneration.from_pretrained(
    args.model_path, torch_dtype=torch.float32, device_map="cpu",
    trust_remote_code=True).eval()

processor = AutoProcessor.from_pretrained(args.model_path, trust_remote_code=True)

# Hook the combined module so the calibration dataset matches QuantTool.load_model(model=vision+mlp_AR).
model.visual = PaddleVLVisionMLPAR(model.visual, model.mlp_AR)
