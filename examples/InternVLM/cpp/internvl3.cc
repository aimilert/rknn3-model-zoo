...
int init_internvl3_model(rknn_app_context_t* app_ctx, const char* llm_model_path, const char* llm_weight_path, const char* vision_model_path, const char* vision_weight_path, rknn3_llm_param* params, int n_params, RKLLMCallback& callback, uint32_t vision_core_mask, uint32_t llm_core_mask)
{
    int ret = 0;

    printf("--> init internvl3 vision model\n");
    ret = init_internvl3_vision(&(app_ctx->vision), vision_model_path, vision_weight_path, vision_core_mask);
    if (ret < 0)
    {
        printf("rknn_init internvl3 vision model fail! ret=%d\n", ret);
        return ret;
    }

    printf("--> init internvl3 llm model\n");
    ret = init_internvl3_llm(&(app_ctx->llm), llm_model_path, llm_weight_path, params, n_params, callback, llm_core_mask);
    if (ret < 0)
    {
        printf("rknn_init internvl3 llm model fail! ret=%d\n", ret);
        return ret;
    }
    ...
    if (ret < 0)
    {
        printf("internvl3 llm/vision internal memory share fail! ret=%d\n", ret);
        return ret;
    }
...
int release_internvl3_model(rknn_app_context_t* app_ctx)
{
    ...
    release_internvl3_vision(&(app_ctx->vision));
    release_internvl3_llm(&(app_ctx->llm));
    return 0;
}
...
int inference_internvl3_model(rknn_app_context_t* app_ctx, image_buffer_t* img, float16* img_embeds, rknn3_llm_multimodal_tensor tensor, int n_inputs, rknn_perf_metrics_t* perf)
{
    ...
    printf("--> inference internvl3 vision model\n");
    int start_us = getCurrentTimeUs();
    ret = inference_internvl3_vision(&(app_ctx->vision), img, img_embeds);
    perf->vision_latency = getCurrentTimeUs()-start_us;  
    if (ret != 0)
    {
        printf("inference internvl3 vision model fail! ret=%d\n", ret);
        return ret;
    }

    printf("--> inference internvl3 llm model\n");  
    ret = inference_internvl3_llm(&(app_ctx->llm), tensor, n_inputs, perf);
    if (ret != 0)
    {
        printf("inference internvl3 llm model fail! ret=%d\n", ret);
        return ret;
    }
