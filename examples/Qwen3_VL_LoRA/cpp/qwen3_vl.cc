...
int inference_qwen3_vl_model_vision(rknn_app_context_t* app_ctx, image_buffer_t* img, float16* img_embeds, rknn_perf_metrics_t* perf)
{
    if (!app_ctx || !img) {
    ...
    int ret = inference_qwen3_vl_vision(&(app_ctx->vision), img, img_embeds,
