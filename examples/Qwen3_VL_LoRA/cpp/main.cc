...
const rknn3_sampling_params SAMPLE_PARAMS = {
    ...
    .top_p = 0.9,
    .temperature = 0.7f,
    .repeat_penalty = 1.0f,   // Please refer to generation_config.json to configure the corresponding parameters. eg, //huggingface.co/Qwen/Qwen3-VL-2B-Instruct/blob/main/generation_config.json
...
// '<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n<|vision_start|><|image_pad|>*x<|vision_end|>这张图片是关于什么的<|im_end|>\n<|im_start|>assistant\n'
/*-------------------------------------------
                Callback Function
-------------------------------------------*/
...
int main(int argc, char **argv)
{
    ...
    char *llm_lora_weight_path     = nullptr;
    ...
    rknn_perf_metrics_t perf_base;
    rknn_perf_metrics_t perf_lora;
    ...
    printf("vocab_info: vocab_size=%d special_bos_id=%d special_eos_id=%d\n", vocab_info.vocab_size,
            vocab_info.n_special_bos_id, vocab_info.n_special_eos_id);
    ...
    // Vision 跑一次，deepstack 供 base / lora 两段 LLM 共用
    ret = inference_qwen3_vl_model_vision(&rknn_app_ctx, &src_image, img_embeds, &perf);
    if (ret != 0) {
        printf("inference qwen3_vl vision fail! ret=%d\n", ret);
        goto out;
    }
    ...
    first_decode = true;
    perf_base = perf;
    ret = inference_qwen3_vl_llm_base(&(rknn_app_ctx.llm), tensor, n_inputs, &perf_base);
    if (ret != 0) {
        printf("inference qwen3_vl llm (base) fail! ret=%d\n", ret);
        goto out;
    }
    printf_perf(&perf_base);

    // 第二段：LoRA 模型推理
    printf("\n========== LoRA model ==========\n");
    first_decode = true;
    perf_lora = perf;
    ret = inference_qwen3_vl_llm_lora(&(rknn_app_ctx.llm), tensor, n_inputs, &perf_lora);
    if (ret != 0) {
        printf("inference qwen3_vl llm (lora) fail! ret=%d\n", ret);
        goto out;
    }
    printf_perf(&perf_lora);
out:
    ret = release_qwen3_vl_model(&rknn_app_ctx);

    if (ret != 0)
    {
        printf("release qwen3_vl model fail! ret=%d\n", ret);
    }
