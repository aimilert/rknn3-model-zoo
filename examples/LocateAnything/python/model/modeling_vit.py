...
def _apply_rope_input_validation(x, freqs_cis):
    assert x.ndim == freqs_cis.ndim, (x.shape, freqs_cis.shape)
    assert x.shape[:-2] == freqs_cis.shape[:-2], (x.shape, freqs_cis.shape)
    assert x.shape[-1] == 2 * freqs_cis.shape[-2], (x.shape, freqs_cis.shape)
...
def apply_rope(
    ...
    Args: (The leading dimensions of all inputs should be the same)
        ...
        freqs_cis: tensor of shape (..., head_dim/2, 2). The last dim stores
            the precomputed cos/sin values for each position in the 2D grid.
    Returns:
    ...
    xq_out = torch.stack(
        (
            xq_[..., 0] * freqs_cos - xq_[..., 1] * freqs_sin,
            xq_[..., 0] * freqs_sin + xq_[..., 1] * freqs_cos,
        ),
        dim=-1,
    ).flatten(-2)
    xk_out = torch.stack(
        (
            xk_[..., 0] * freqs_cos - xk_[..., 1] * freqs_sin,
            xk_[..., 0] * freqs_sin + xk_[..., 1] * freqs_cos,
        ),
        dim=-1,
    ).flatten(-2)
...
class Rope2DPosEmb(nn.Module):
    ...
    def _precompute_freqs_cis(self, device: torch.device) -> torch.Tensor:
        ...
        Return: float tensor of shape (max_height, max_width, dim//2, 2) and value:
        ...
        x_cis = torch.stack((torch.cos(x_freqs), torch.sin(x_freqs)), dim=-1)  # N, C/4, 2
        y_cis = torch.stack((torch.cos(y_freqs), torch.sin(y_freqs)), dim=-1)  # N, C/4, 2
        # N, C/4, 2, 2
        freqs_cis = torch.cat(
            [x_cis.unsqueeze(dim=-2), y_cis.unsqueeze(dim=-2)], dim=-2
        )
        # max_height, max_width, C/2, 2
        freqs_cis = freqs_cis.reshape(self.max_height, self.max_width, -1, 2)
        return freqs_cis
    ...
    def get_freqs_cis(self, grid_hws: torch.Tensor) -> torch.Tensor:
        ...
        """
        # freqs_cis 是普通属性(非 registered buffer), model.cpu() 不会移动它,
        # 量化后在 cuda 上缓存、导出在 cpu 上运行会导致 device mismatch, 这里按需重算/搬移
        if self.freqs_cis is None or self.freqs_cis.device != grid_hws.device:
            self.freqs_cis = self._precompute_freqs_cis(grid_hws.device)

        shapes = grid_hws.tolist()
        # During torch.export / torch.onnx.export tracing, registered-buffer
        # values become symbolic ints (SymInt).  Guarding on them (e.g. the
        # assert below) raises GuardOnDataDependentSymNode.  Skip the assert
        # when shapes contain non-concrete values; the check is purely
        # defensive and has no effect on the computed output.
        ...
        freqs_cis = torch.cat(
            [self.freqs_cis[:h, :w].reshape(-1, self.dim // 2, 2) for h, w in shapes],
            dim=0,
        )
        return freqs_cis
...
class MoonVitEncoder(nn.Module):
    ...
    def forward(
        ...
        cumsum_dtype = grid_hws.dtype if torch.onnx.is_in_onnx_export() else torch.int32
        cu_seqlens = lengths.cumsum(dim=0, dtype=cumsum_dtype)
