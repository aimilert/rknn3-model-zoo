...
#include "rknn_nanbeige4_2_llm.h"
...
int tokenizer_callback(void *userdata, const char *text, int32_t text_len, int32_t *tokens, int32_t n_tokens_max)
{
    ...
    printf("text: %.*s, tokens: [", text_len, text);
...
int main(int argc, char **argv)
{
    ...
    // RKNN Context
    rknn_nanbeige4_2_llm_context rknn_app_ctx;
    memset(&rknn_app_ctx, 0, sizeof(rknn_nanbeige4_2_llm_context));
    ...
    printf("--> init nanbeige4_2 llm model\n");  
    ret = init_nanbeige4_2_llm(&rknn_app_ctx, model_path, weight_path, &params, n_params, callback, core_mask);
    if (ret != 0)
    {
        printf("init_nanbeige4_2_llm fail! ret=%d model_path=%s weight_path=%s\n", ret, model_path, weight_path);
        goto out;
    }
    ...
    printf("--> inference nanbeige4_2 llm model\n");  
    ret = inference_nanbeige4_2_llm(&rknn_app_ctx, tensor, n_inputs, &perf);
    if (ret != 0)
    {
        printf("inference nanbeige4_2 llm fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ret = release_nanbeige4_2_llm(&rknn_app_ctx);
    if (ret != 0)
    {
        printf("release nanbeige4_2 llm fail! ret=%d\n", ret);
    }
