...
'''
Remove the chat template from the exported llm config file. The chat template for Nanbeige is not supported by RKNN Session.
'''
def remove_chat_template(config_path):
    import pickle
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description="Export Nanbeige/Nanbeige4.2-3B llm configuration and onnx model for RKNN")
    parser.add_argument("--model_path", type=str, help="model path or name", required=False, default="Nanbeige/Nanbeige4.2-3B")
    parser.add_argument("--export_llm_path", type=str, help="export llm onnx model path", required=False, default="../model/llm/Nanbeige4.2-3B.onnx")
    ...
    # Fix transformers 5.x compatibility: 5.x normalizes rope_scaling=null to
    # {'rope_type': 'default', ...} which breaks the custom modeling code that
    # checks `if self.config.rope_scaling is None`. Restore None when no scaling.
    if config.rope_scaling is not None and isinstance(config.rope_scaling, dict) \
            and config.rope_scaling.get('rope_type', config.rope_scaling.get('type')) == 'default':
    ...
    export_llm_config(args.model_path, config_path, chat_context, prompt)
    remove_chat_template(config_path)
