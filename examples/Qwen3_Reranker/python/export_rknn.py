...
ONNX_MODEL = '../model/llm/Qwen3-Reranker-0.6B.onnx'
LLM_CONFIG = '../model/llm/Qwen3-Reranker-0.6B.config.pkl'
RKNN_MODEL = '../model/llm/Qwen3-Reranker-0.6B.rknn'
DATASET_PATH = None

if __name__ == '__main__':

    from argparse import ArgumentParser
    parser = ArgumentParser(description="Export Qwen/Qwen3-Reranker to RKNN model") 
    ...
    # Load model
    print('--> Loading model')
    ret = rknn.load_llm(model=args.onnx_path, config=args.config, seq_lens = [1, 128])
    if ret != 0:
