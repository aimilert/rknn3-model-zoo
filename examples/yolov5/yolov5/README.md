...
**特点：**
- 标准的 YOLOv5 模型，无需对原始模型做特别修改
- 输出为 3 个特征图（stride=8/16/32 的检测头输出）
- 支持两种后处理方式（见下文）

#### 模式 A：配合后处理插件使用（⭐ 推荐单核场景）
**使用方式：**
- 需要配合后处理插件 `libpostprocess_yolov5_rk182x.so` 使用
- 通过 `rknn3_register_custom_ops_plugins()` API 加载插件
- 后处理在 RK182x 协处理器中的 CPU 执行

**优点：**
- ✅ 不需要修改原始 YOLOv5 模型，使用简单
- ✅ 单核性能有较大优势
- ✅ 减少主控端与协处理器之间的数据传输

**缺点：**
- ⚠️ 后处理在协处理器 CPU 上执行，多核时可能存在CPU资源竞争
...
**使用方式：**
- 不需要后处理插件 `libpostprocess_yolov5_rk182x.so`
- 模型后处理已内置，推理时自动执行
- 直接输出检测结果
...
- yolov5n_rknn3.onnx: `https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov5/yolov5n_rknn3.onnx`
- yolov5s.onnx: `https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov5/yolov5s.onnx`
- yolov5s_rknn3.onnx: `https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov5/yolov5s_rknn3.onnx`
...
编译成功后会生成 `libpostprocess_yolov5_rk182x.so`
...
编译完成后，会在以下目录生成文件：
- **构建目录：** `build/build_rknn_yolov5_demo_rk3588_linux_aarch64_Release/`
- **安装目录：** `install/rk3588_linux_aarch64/rknn_yolov5_demo/`（推荐使用）

安装目录包含完整的运行包：
- `rknn_yolov5_demo`：单张图片推理程序
...
编译安装后，程序会被安装到 `install/rk3588_linux_aarch64/rknn_yolov5_demo/` 目录，结构如下：
```
install/rk3588_linux_aarch64/rknn_yolov5_demo/
├── rknn_yolov5_demo              # 主程序
├── dataset_eval                  # 数据集评估程序
├── model/                        # 模型目录
│   ├── bus.jpg
...
**通用命令格式：**
```bash
./rknn_yolov5_demo <model_path> <weight_path> <image_path> <core_mask> [postprocess_plugin_path]
```
...
./rknn_yolov5_demo model/yolov5n.rknn model/yolov5n.weight model/bus.jpg 0x1 lib/libpostprocess_yolov5_rk182x.so
```

**模式 B：不带_rknn3模型，主控端后处理（❌ 不推荐）**
```bash
# 不使用插件，后处理在主控端 CPU 执行
./rknn_yolov5_demo model/yolov5n.rknn model/yolov5n.weight model/bus.jpg 0x1
```

**模式 C：带_rknn3模型（⭐ 推荐多核）**
```bash
# 使用内置后处理的模型，不需要插件
./rknn_yolov5_demo model/yolov5n_rknn3.rknn model/yolov5n_rknn3.weight model/bus.jpg 0x1
```

**核心代码说明（模式 A）：**

在 `yolov5.cc` 的 `init_yolov5_model()` 函数中，通过传入 `postprocess_plugin_path` 参数来启用后处理插件：
...
**重要说明：**
- 后处理插件 `libpostprocess_yolov5_rk182x.so` 配合**不带_rknn3后缀**的模型使用
- 编译安装后插件位于 `lib/libpostprocess_yolov5_rk182x.so`
...
控制台输出示例：
```
person @ (208 242 286 509) 0.828
person @ (110 235 203 525) 0.822
person @ (483 225 565 513) 0.588
bus @ (113 147 569 455) 0.523
```
...
编译完成后会在 `install/rk3588_linux_aarch64/rknn_yolov5_demo/` 目录生成完整的安装包。
...
./dataset_eval model/yolov5n.rknn model/yolov5n.weight 0x1 lib/libpostprocess_yolov5_rk182x.so
```

**模式 B：不带_rknn3模型，主控端后处理（不推荐）**
```bash
# 不使用插件，主控端后处理（数据传输开销大）
./dataset_eval model/yolov5n.rknn model/yolov5n.weight 0x1
```
...
```bash
# 从板端拉取结果文件
adb pull /userdata/rknn_yolov5_demo/results_rknn.json ./
```
...
后处理插件 `libpostprocess_yolov5_rk182x.so` 是 RKNN3 平台的自定义算子扩展机制，用于**模式 A**（不带_rknn3模型 + 后处理插件）。

**工作原理：**
- 配合**不带_rknn3后缀**的标准 YOLOv5 模型使用
...
├── yolov5_postprocess.c       # YOLOv5 后处理核心算法
...
编译成功后会生成 `libpostprocess_yolov5_rk182x.so` 动态库文件。
...
rknn3_load_model_from_path(ctx, "yolov5s.rknn", "yolov5s.weight");
...
// 输出格式: [N, 1, 256, 6] - 已完成后处理的检测结果
```

**注意事项：**
1. ⚠️ **插件配合不带_rknn3后缀的模型使用**（如 `yolov5s.rknn`，而非 `yolov5s_rknn3.rknn`）
...
2. **模型与插件不匹配**：
   - ✅ 正确：`yolov5s.rknn` + `libpostprocess_yolov5_rk182x.so`
   - ❌ 错误：`yolov5s_rknn3.rknn` + 插件（带_rknn3模型不需要插件）
