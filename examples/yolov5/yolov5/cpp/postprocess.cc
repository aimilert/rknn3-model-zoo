...
int post_process_after_exYoloPostProcess(rknn_app_context_t *app_ctx, void *outputs, rknn3_tensor_attr output_attrs, letterbox_t *letter_box, object_detect_result_list *od_results)
{
    ...
    for(int i = 0; i < K; i++)
    {
        ...
        if(score > 0)
        {
            ...
            int cls_id = (int)outputfp32[i*6 + 1];
            ...
            //        last_count, x1, y1, x2, y2, obj_conf, cls_id);
            last_count++;
        }
    }
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        grid_h = app_ctx->outputs[i].attr->shape[2];
        grid_w = app_ctx->outputs[i].attr->shape[3];
        stride = model_in_h / grid_h;

        if (app_ctx->is_quant)
        {
            validCount += process_i8((int8_t *)_outputs[i].mem->virt_addr, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride,
                                     filterBoxes, objProbs, classId, conf_threshold,
                                     app_ctx->outputs[i].attr->qnt_info.zero_point, app_ctx->outputs[i].attr->qnt_info.scale);
        }
        else
        {   
            float *input = nullptr;
            //convert from fp16 to fp32
            if(_outputs[i].attr->dtype == RKNN3_TENSOR_FLOAT16){
                int size = getTensorSize(_outputs[i]);
                input = (float*) malloc(size * sizeof(float));
                convert_fp16_to_fp32((float16*)_outputs[i].mem->virt_addr, input, size);
            }
            else {
                input = (float *)_outputs[i].mem->virt_addr;
            }

            validCount += process_fp32(input, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride,
                                       filterBoxes, objProbs, classId, conf_threshold);
