...
int init_ui_tars_vl_model(rknn_app_context_t* app_ctx, const char* llm_model_path, const char* llm_weight_path, const char* vision_model_path, const char* vision_weight_path, rknn3_llm_param* params, int n_params, RKLLMCallback& callback, uint32_t vision_core_mask, uint32_t llm_core_mask)
{
    int ret = 0;

    printf("--> init ui_tars_vl vision model\n");
    ret = init_ui_tars_vl_vision(&(app_ctx->vision), vision_model_path, vision_weight_path, vision_core_mask);
    if (ret < 0)
    {
        printf("rknn_init ui_tars_vl vision model fail! ret=%d\n", ret);
        return ret;
    }

    printf("--> init ui_tars_vl llm model\n");
    ret = init_ui_tars_vl_llm(&(app_ctx->llm), llm_model_path, llm_weight_path, params, n_params, callback, llm_core_mask);
    if (ret < 0)
    {
        printf("rknn_init ui_tars_vl llm model fail! ret=%d\n", ret);
        return ret;
    }
    ...
    if (ret < 0)
    {
        printf("ui_tars_vl llm/vision internal memeory share fail! ret=%d\n", ret);
        return ret;
    }
...
int release_ui_tars_vl_model(rknn_app_context_t* app_ctx)
{
    release_internal_share(app_ctx);
    release_ui_tars_vl_vision(&(app_ctx->vision));
    release_ui_tars_vl_llm(&(app_ctx->llm));
    return 0;
}

int inference_ui_tars_vl_model(rknn_app_context_t* app_ctx, image_buffer_t* img, float16* img_embeds, rknn3_llm_multimodal_tensor tensor, int n_inputs, rknn_perf_metrics_t* perf)
{
    int ret;
    ...
    printf("--> inference ui_tars_vl vision model\n");
    int start_us = getCurrentTimeUs();
    ret = inference_ui_tars_vl_vision(&(app_ctx->vision), img, img_embeds);
    perf->vision_latency = getCurrentTimeUs() - start_us;
    if (ret != 0)
    {
        printf("inference ui_tars_vl vision model fail! ret=%d\n", ret);
        return ret;
    }

    printf("--> inference ui_tars_vl llm model\n");  
    ret = inference_ui_tars_vl_llm(&(app_ctx->llm), tensor, n_inputs, perf);

    if (ret != 0)
    {
        printf("inference ui_tars_vl llm model fail! ret=%d\n", ret);
        return ret;
    }
