...
class LinearAttentionFunction_Chunk(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query, key, value, g, beta, use_qk_l2norm_in_kernel=True):
        # 这里实现你的线性注意力计算逻辑
        # ctx 可以用于保存一些值（如果需要反向传播，但目前我们只关心导出）
        attn_out, status = torch_chunk_gated_delta_rule_rkpack(
            query=query,
            key=key,
            value=value,
            g=g,
    ...
    @staticmethod
    def symbolic(graph, q, k, v, g, b, norm_b):
        # 关键：使用 g.op 生成自定义节点
        # 第一个参数格式为 "域名::算子名"，例如 "com.mycompany::linear_attention"
        # 后面依次传入输入张量，并通过 outputs 指定输出数量
        return graph.op("rknn::exLinearAttention", q, k, v, g, b, norm_b, outputs=1)


def torch_chunk_gated_delta_rule_rkpack(
    query,
    key,
    value,
    g,
    ...
    output_final_state=False,
    use_qk_l2norm_in_kernel=False):
    # 2301 inverse also 2301
    query = query.permute(2, 3, 0, 1).contiguous()
    key = key.permute(2, 3, 0, 1).contiguous()
    value = value.permute(2, 3, 0, 1).contiguous()
    g = g.permute(0,2,3,1).contiguous().squeeze(0)
    beta = beta.permute(0,2,3,1).contiguous().squeeze(0)
    ...
    core_attn_out = core_attn_out.permute(2, 3, 0, 1).contiguous()
    last_recurrent_state = last_recurrent_state.permute(1, 2, 0, 3).contiguous() if last_recurrent_state is not None else None
    return core_attn_out, last_recurrent_state
...
def torch_recurrent_gated_delta_rule(
    ...
    for i in range(sequence_length):
        ...
        if g_t.abs().max() > 1e2:  # to prevent overflow
            print("Warning: g_t is too large, may cause overflow. Max of g_t: ", g_t.abs().max())

        beta_t = beta[:, :, i].unsqueeze(-1)

        last_recurrent_state = last_recurrent_state * g_t
        if last_recurrent_state.abs().max() > 1e2:  # to prevent overflow
            print("Warning: last_recurrent_state* g.exp() is too large, may cause overflow. Max of last_recurrent_state: ", last_recurrent_state.abs().max())
        
        kv_mem = (last_recurrent_state * k_t.unsqueeze(-1)).sum(dim=-2)
        if kv_mem.abs().max() > 1e2:  # to prevent overflow
            print("Warning: kv_mem is too large, may cause overflow. Max of kv_mem: ", kv_mem.abs().max())

        delta = (v_t - kv_mem) * beta_t
        if delta.abs().max() > 1e2:  # to prevent overflow
            print("Warning: delta is too large, may cause overflow. Max of delta: ", delta.abs().max())
...
class LinearAttentionFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query, key, value, g, beta, Tc, use_qk_l2norm_in_kernel=True):
        # 这里实现你的线性注意力计算逻辑
        # ctx 可以用于保存一些值（如果需要反向传播，但目前我们只关心导出）
        attn_out, status = torch_recurrent_gated_delta_rule_rkpack(
            query=query,
            key=key,
            value=value,
            g=g,
    ...
    @staticmethod
    def symbolic(graph, q, k, v, g, b, Tc, qk_norm):
        # 关键：使用 g.op 生成自定义节点
        # 第一个参数格式为 "域名::算子名"，例如 "com.mycompany::linear_attention"
        # 后面依次传入输入张量，并通过 outputs 指定输出数量
        return graph.op("rknn::exLinearAttention", q, k, v, g, b, Tc, qk_norm_i=qk_norm, outputs=1)


def torch_recurrent_gated_delta_rule_rkpack(
    query, key, value, g, beta, initial_state, output_final_state, use_qk_l2norm_in_kernel=False
):
    query = query.permute(2, 3, 0, 1).contiguous()
    key = key.permute(2, 3, 0, 1).contiguous()
    ...
    if initial_state is not None:
        initial_state = initial_state.permute(2,0,1,3).contiguous()
...
class Qwen3_5GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        if use_precomputed_states or (cache_params is not None and conv_state is not None):
        ...
        if not use_precomputed_states:
            # TO RK layout, only support batch == 1
            query = query.permute(2, 3, 0, 1).contiguous()
            key = key.permute(2, 3, 0, 1).contiguous()
            value = value.permute(2, 3, 0, 1).contiguous()
            g = g.unsqueeze(0).permute(0, 3, 1, 2).contiguous()
            beta = beta.unsqueeze(0).permute(0, 3, 1, 2).contiguous()
            
            ### 导出模型使用
            # core_attn_out = LinearAttentionFunction.apply(
            #     query, key, value, g, beta,  Tc, True
            # )
            # last_recurrent_state = None

            ### pytorch 推理使用
            core_attn_out, last_recurrent_state = torch_chunk_gated_delta_rule_rkpack(
                query,
                key,
                value,
                g=g,
            ...
            core_attn_out = core_attn_out.permute(2, 3, 0, 1).contiguous()
            last_recurrent_state = last_recurrent_state.permute(2, 0, 1, 3).contiguous() if last_recurrent_state is not None else None
            ...
            g = g.unsqueeze(0).permute(0, 3, 1, 2).contiguous()
            beta = beta.unsqueeze(0).permute(0, 3, 1, 2).contiguous()
            recurrent_state = recurrent_state.permute(1,2,0,3).contiguous() if recurrent_state is not None else None
            ...
            ### pytorch 推理使用
            core_attn_out, last_recurrent_state = torch_recurrent_gated_delta_rule_rkpack(
                query,
                key,
                value,
                g=g,
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if outputs.past_key_values is not None:
            all_min = [v.min() for v in outputs.past_key_values.recurrent_states if v is not None]
            all_max = [v.max() for v in outputs.past_key_values.recurrent_states if v is not None]
            print("attention_mask shape:", attention_mask.shape, ';', 'Past key values min:', min(all_min), 'max:', max(all_max))
