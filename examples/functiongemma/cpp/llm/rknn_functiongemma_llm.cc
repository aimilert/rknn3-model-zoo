...
static int query_input_tensors(rknn_functiongemma_llm_context* llm_ctx)
{
  int                    ret;
  ...
  // Collect indices of ext inputs: rope_cos_cache/rope_sin_cache
  ...
  for (int i = 0; i < llm_ctx->n_input_tensors; ++i) {
    ...
    if (need_rope_cache && (strstr(name, "rope_cos_cache") || strstr(name, "rope_sin_cache"))) {
      n_ext++;
    }
  }

  if (n_ext == 0) {
    printf("no ext input tensors (%s) found\n",
...
int init_functiongemma_llm(rknn_functiongemma_llm_context* llm_ctx, const char* model_path, const char* weight_path,
                    rknn3_llm_param* params, int n_params, RKLLMCallback* callback, uint32_t core_mask)
{
  ...
  if (ret != RKNN3_SUCCESS) {
    ...
    release_functiongemma_llm(llm_ctx);
    return ret;
  }
  ...
  ret = rknn3_session_set_chat_template(llm_ctx->rknn_sess, "", "<start_of_turn>user\n", "<end_of_turn>\n<start_of_turn>model\n");
  if (ret != RKNN3_SUCCESS) {
...
int release_functiongemma_llm(rknn_functiongemma_llm_context* llm_ctx)
{
  if (!llm_ctx) {
    return 0;
  }
...
int inference_functiongemma_llm(rknn_functiongemma_llm_context* llm_ctx, rknn3_llm_multimodal_tensor tensor, int32_t max_new_tokens,
