...
def causal_gemma3_to_onnx(model, args):
    import torch
    ...
    class ModelWrapper(torch.nn.Module):
        ...
        def forward(self, input_ids, attention_mask_global, position_ids_global,
            ...
            # no PLE: 直接传 input_ids 给 model（gemma3 无 per_layer_inputs）
            outputs = self.model(
                input_ids=input_ids,
                attention_mask=None,
                position_ids=None,
    ...
    for key in logit_keep_keys:
        if key in _forward_func.__code__.co_varnames:
            ...
            break
    if logit_keep_key:
        # 只留指定 token 的 logits。dummy 用 0（非负），index_select 不支持负索引；
        # 部署时传非负正索引（prefill=seq_len-1, decode=0）。
        num_logits_to_keep = torch.tensor(0, dtype=torch.long).reshape(1)
