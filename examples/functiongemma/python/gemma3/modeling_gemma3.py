...
class Gemma3DecoderLayer(nn.Module):
    def __init__(self, config: Gemma3TextConfig, layer_idx: int):
        ...
        # residual_scale: 缩放残差流抑制 FP16 溢出，对齐 conversion_scripts 的 --residual_scale 16。
        self.residual_scale = 16.0
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Select the global/local attention mask based on the layer type (gemma4 session contract).
        attention_mask = attention_mask_local if self.is_sliding else attention_mask_global
...
class Gemma3TextModel(Gemma3PreTrainedModel):
    ...
    def forward(
        ...
        attention_mask_global: Optional[torch.Tensor] = None,
        attention_mask_local: Optional[torch.Tensor] = None,
        position_ids_global: Optional[torch.LongTensor] = None,
        position_ids_local: Optional[torch.LongTensor] = None,
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # 缩放残差流抑制 FP16 溢出，对齐 conversion_scripts 的 hidden_states / residual_scale。
        # fuse_gemma3_embed_scale 已把 embed_scale 折进权重，这里再除以 residual_scale=16。
        inputs_embeds = inputs_embeds / self.residual_scale
        ...
        # Separate global/local position_ids for the two RoPE streams (gemma4 session contract).
        ...
        # Convert 2D global/local attention masks to 4D causal masks. When the split
        # masks are not provided, fall back to the single causal_mask so behavior is
        # unchanged for callers that only pass `attention_mask`.
        if attention_mask_global is not None and attention_mask_global.dim() == 2:
            causal_mask_global = self._prepare_4d_causal_attention_mask_with_cache_position(
            ...
            causal_mask_global = attention_mask_global if attention_mask_global is not None else causal_mask

        if attention_mask_local is not None and attention_mask_local.dim() == 2:
            causal_mask_local = self._prepare_4d_causal_attention_mask_with_cache_position(
            ...
            causal_mask_local = attention_mask_local if attention_mask_local is not None else causal_mask

        # create position embeddings (two streams) to be shared across the decoder layers
        position_embeddings_global = self.rotary_emb(hidden_states, position_ids_global)
        position_embeddings_local = self.rotary_emb_local(hidden_states, position_ids_local)
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if isinstance(logits_to_keep, int):
            ...
            # logits_to_keep 是张量时，用 index_select 沿 seq 维取，生成 Gather(axis=1)
            # （避免高级索引 hidden_states[:, tensor, :] 产生 GatherND，RKNN 不支持）。
            # 部署时 num_logits_to_keep 恒为非负正索引（prefill=seq_len-1, decode=0），
            # 无需负索引转正，直接 Gather。输入为 int64，无需 Cast。
            logits = self.lm_head(torch.index_select(hidden_states, 1, logits_to_keep))
