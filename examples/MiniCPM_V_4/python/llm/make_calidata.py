...
parser.add_argument('--model_path', type=str, default='openbmb/MiniCPM-V-4', help='model path', required=False)
...
for idx, data in enumerate(tqdm(datasets)):
    ...
    with torch.inference_mode():
        temp = capture_module_input(
            model.llm,
            lambda: model.chat(msgs=msgs, image=image, tokenizer=tokenizer)
        )
