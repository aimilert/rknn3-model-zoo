...
DEFAULT_OUTPUT = '../model/yolo26m_pose.rknn'
...
YOLO26_POSE_OUTPUT_SPECS = [
    ('box_1', YOLO26_POSE_RAW_OUTPUTS[0], False),
    ('score_1', YOLO26_POSE_RAW_OUTPUTS[1], False),
    ('pos_1', YOLO26_POSE_RAW_OUTPUTS[2], False),
    ('box_2', YOLO26_POSE_RAW_OUTPUTS[3], False),
    ('score_2', YOLO26_POSE_RAW_OUTPUTS[4], False),
    ('pos_2', YOLO26_POSE_RAW_OUTPUTS[5], False),
    ('box_3', YOLO26_POSE_RAW_OUTPUTS[6], False),
    ('score_3', YOLO26_POSE_RAW_OUTPUTS[7], False),
    ('pos_3', YOLO26_POSE_RAW_OUTPUTS[8], False),
]
...
Examples:
  ...
  python3 convert.py --onnx model.onnx --platform rk1820 --dtype i8

  # With custom output path and dataset
  python3 convert.py --onnx model.onnx --platform rk1820 --dtype i8 --output yolo26n.rknn --dataset dataset.txt

  # Enable inference test on remote device (platform is also the target)
  python3 convert.py --onnx model.onnx --platform rk3572 --dtype i8 --device-id 172.16.15.199:5555

  # Enable accuracy analysis on RKNN3 platform (requires --device-id)
  python3 convert.py --onnx model.onnx --platform rk3572 --dtype i8 --device-id 172.16.15.199:5555 --accuracy-analysis

  # Enable inference test with local simulator
  python3 convert.py --onnx model.onnx --platform rk1820 --dtype i8 --inference
        '''
    )
    ...
    parser.add_argument('--dtype', choices=['w8a8', 'fp', 'w4a16', 'w8a16', 'w6a16'], default='w8a8',
                        help='Data type: w8a8 (quantized) or fp (float). Default: w8a8')
    ...
    return args.onnx, args.platform, do_quant, args.dtype, args.output, args.dataset, args.core_num, do_inference, args.device_id, args.accuracy_analysis
...
def add_sigmoid_to_score_heads(onnx_path):
    ...
    missing_outputs = [raw_name for _, raw_name, _ in YOLO26_POSE_OUTPUT_SPECS if raw_name not in available_values]
    if missing_outputs:
    ...
    for output_name, raw_name, needs_sigmoid in YOLO26_POSE_OUTPUT_SPECS:
        if needs_sigmoid:
...
if __name__ == '__main__':
    model_path, platform, do_quant, dtype, output_path, dataset_path, core_num, do_inference, device_id, accuracy_analysis = parse_arg()
    ...
    if do_quant:
        config_params['quantized_dtype'] = dtype

    subgraph = None
    if dtype=='w8a8' and platform == 'rk1820':
        subgraph_inputs1 = [
        ...
        subgraph = [[subgraph_inputs1, subgraph_outputs1, 'w16a16'], [subgraph_inputs2, subgraph_outputs2, 'w16a16'], [subgraph_inputs3, subgraph_outputs3, 'w16a16'],
         [subgraph_inputs4, subgraph_outputs4, 'w16a16'], [subgraph_inputs5, subgraph_outputs5, 'w16a16'], [subgraph_inputs6, subgraph_outputs6, 'w16a16'], [subgraph_inputs7, subgraph_outputs7, 'w16a16'],]
    ...
    # Load model
    # Outputs are 9 detection heads: 3 classification heads + 3 regression heads + 3 keypoint heads
    print('--> Loading model')

    # RKNN3 1.0.5b10 may abort in int8 codegen when fusing output Sigmoid as
    # a LUT. Keep score heads as logits for quantized RK1820 builds; runtime
    # postprocess already applies sigmoid when it sees non-probability scores.
