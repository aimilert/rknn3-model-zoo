...
static const int COCO_PERSON_CATEGORY_ID = 1;
static const float COCO_CONF_THRESH = 0.001f;
static const float COCO_NMS_THRESH = 0.70f;
static const char *DEFAULT_DATASET_LIST = "/userdata/coco_dataset_keypoint_test_path.txt";
static const char *DEFAULT_OUTPUT_JSON = "results_rknn_keypoints.json";
...
struct DetectionResult {
    ...
    double bbox[4];
    double score;
    double keypoints[OBJ_KEYPOINT_NUM][3];
};
...
static bool parse_args(int argc, char **argv, EvalConfig *cfg)
{
    ...
    cfg->conf_thresh = COCO_CONF_THRESH;
    cfg->nms_thresh = COCO_NMS_THRESH;
...
static void write_results_to_json(const std::vector<DetectionResult> &results, const std::string &output_file)
{
    ...
    for (const auto &res : results)
    {
        ...
        json_fp << "\"category_id\":" << COCO_PERSON_CATEGORY_ID << ",";
        json_fp << "\"bbox\":[";
...
int main(int argc, char **argv)
{
    ...
    printf("YOLO26 pose COCO keypoints eval\n");
    printf("  model        : %s\n", cfg.model_path);
    printf("  weight       : %s\n", cfg.weight_path);
    ...
    for (size_t img_idx = 0; img_idx < image_paths.size(); img_idx++)
    {
        ...
        if (ret != 0)
        {
            printf("inference_yolo26_model fail! ret=%d image_path=%s\n", ret, image_path.c_str());
        ...
        for (int i = 0; i < od_results.count; i++)
        {
            ...
            for (int k = 0; k < OBJ_KEYPOINT_NUM; k++)
            {
                res.keypoints[k][0] = clamp_double(det->keypoints[k].x_float, 0.0, img_w);
                res.keypoints[k][1] = clamp_double(det->keypoints[k].y_float, 0.0, img_h);
                res.keypoints[k][2] = det->keypoints[k].prop;
            }
