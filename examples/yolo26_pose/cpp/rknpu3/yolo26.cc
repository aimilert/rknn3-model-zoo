...
static size_t tensor_type_size(rknn3_tensor_type type)
{
    switch (type)
    {
...
static std::string tensor_shape_string(const rknn3_tensor_attr *attr)
{
    std::string shape;
    for (uint32_t i = 0; i < attr->n_dims; i++)
    {
        ...
        shape += std::to_string(attr->shape[i]);
    }
    return shape;
}

static const char *pose_output_role(int index, int output_count)
{
    if (output_count == 6)
    {
...
int inference_yolo26_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results, double* inter_time, float nms_thresh, float bos_thresh)
{
    int ret = 0;
    image_buffer_t dst_img;
    letterbox_t letter_box;
    const float nms_threshold = nms_thresh;      // 默认的NMS阈值
    const float box_conf_threshold = bos_thresh; // 默认的置信度阈值
    int bg_color = 114;
    ...
    *inter_time    += npu_runtime;
    ...
    if(app_ctx->io_num.n_output == 1 && output_attrs.n_dims == 3 &&
       (output_attrs.shape[1] >= END2END_DET_ELEM_NUM || output_attrs.shape[2] >= END2END_DET_ELEM_NUM))
    {
