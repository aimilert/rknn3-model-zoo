    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            for (int c= 0; c< OBJ_CLASS_NUM; c++){
                int8_t score_i8 = score_tensor[offset];
                        ...
                        bool score_is_logits)
{
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            float max_score = 0;
            for (int c= 0; c< OBJ_CLASS_NUM; c++){
                float score = score_tensor[offset];
                if (score_is_logits)
                {
                    score = sigmoid(score);
                }
...
int post_process_after_exYoloPostProcess(rknn_app_context_t *app_ctx, void *outputs, rknn3_tensor_attr output_attrs, letterbox_t *letter_box, object_detect_result_list *od_results)
{
    ...
    for(int i = 0; i < K; i++)
    {
        const float* det = &outputfp32[i * 6];

        // Support both common layouts:
        //  - [score, cls, x1, y1, x2, y2] (used by some RKNN postprocess plugins)
        //  - [x1, y1, x2, y2, score, cls] (Ultralytics end2end ONNX `output0`)
        float score = 0.0f;
        int id = -1;
        float x1 = 0.0f, y1 = 0.0f, x2 = 0.0f, y2 = 0.0f;

        bool layout_score_first = (det[0] >= 0.0f && det[0] <= 1.0f) && (det[1] >= -1.0f && det[1] <= (float)(OBJ_CLASS_NUM + 1));
        bool layout_score_last  = (det[4] >= 0.0f && det[4] <= 1.0f) && (det[5] >= -1.0f && det[5] <= (float)(OBJ_CLASS_NUM + 1));
...
{
    int size = 0;
    for (int i = 0; i < tensor.attr->n_dims; i++)
    {
        if (i == 0)
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // default 3 branch
    int dfl_len = app_ctx->outputs[0].attr->shape[1] / 4;
    int output_per_branch = app_ctx->io_num.n_output / 3;
    for (int i = 0; i < 3; i++)
    {
        int box_idx = i*output_per_branch;
        int score_idx = i*output_per_branch + 1;
        int sum_idx = i*output_per_branch + 2;

        rknn3_tensor_type dtype = _outputs[box_idx].attr->dtype;

        grid_h = app_ctx->outputs[box_idx].attr->shape[2];
        grid_w = app_ctx->outputs[box_idx].attr->shape[3];
        bool score_is_logits = strstr(app_ctx->outputs[score_idx].attr->name, "sigmoid") == nullptr;
        ...
        if (app_ctx->is_quant)
        {
            ...
            {
                score_sum = _outputs[i*output_per_branch + 2].mem->virt_addr;
                score_sum_zp = app_ctx->outputs[i*output_per_branch + 2].attr->qnt_info.zero_point;
                score_sum_scale = app_ctx->outputs[i*output_per_branch + 2].attr->qnt_info.scale;
            }

            validCount += process_i8((int8_t *)_outputs[box_idx].mem->virt_addr, app_ctx->outputs[box_idx].attr->qnt_info.zero_point, app_ctx->outputs[box_idx].attr->qnt_info.scale,
                                     (int8_t *)_outputs[score_idx].mem->virt_addr, app_ctx->outputs[score_idx].attr->qnt_info.zero_point, app_ctx->outputs[score_idx].attr->qnt_info.scale,
                                     (int8_t *)score_sum, score_sum_zp, score_sum_scale,
                                     grid_h, grid_w, stride, dfl_len,
                                     filterBoxes, objProbs, classId, conf_threshold, score_is_logits);
        }
        else
        {
            ...
            //convert score_sum from fp16 to fp32
            if (output_per_branch == 3)
            {
                if(_outputs[sum_idx].attr->dtype == RKNN3_TENSOR_FLOAT16){
                    ...
                    score_sum_tensor = (float *)_outputs[sum_idx].mem->virt_addr;
                }
