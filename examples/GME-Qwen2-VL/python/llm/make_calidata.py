...
parser.add_argument('--model_path', type=str, default='iic/gme-Qwen2-VL-2B-Instruct', help='model path', required=False)
...
for idx, data in enumerate(tqdm(datasets)):
    ...
    with torch.inference_mode():
        temp = capture_module_input(
            model.model,
            lambda: model.get_text_embeddings(texts=data['input'])
        )
