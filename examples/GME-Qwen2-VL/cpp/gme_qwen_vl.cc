...
int init_gme_qwen_vl_model(rknn_app_context_t* app_ctx, const char* vision_model_path, const char* vision_weight_path, rknn3_llm_param* params, int n_params, RKLLMCallback& callback, uint32_t vision_core_mask, uint32_t llm_core_mask)
{
    ...
    printf("--> init gme_qwen_vl vision model\n");
    ret = init_gme_qwen_vl_vision(&(app_ctx->vision), vision_model_path, vision_weight_path, vision_core_mask);
    if (ret < 0)
    {
        printf("rknn_init gme_qwen_vl vision model fail! ret=%d\n", ret);
        return ret;
    }

    printf("--> init gme_qwen_vl llm session\n");
    ret = init_gme_qwen_vl_llm_session(&(app_ctx->llm), params, n_params, callback);

    if (ret < 0)
    {
        printf("rknn_init gme_qwen_vl llm model fail! ret=%d\n", ret);
        return ret;
    }
...
int release_gme_qwen_vl_model(rknn_app_context_t* app_ctx)
{
    ...
    release_gme_qwen_vl_vision(&(app_ctx->vision));
    release_gme_qwen_vl_llm(&(app_ctx->llm));
    return 0;
}

int inference_gme_qwen_vl_model(rknn_app_context_t* app_ctx, image_buffer_t* img, float16* img_embeds, rknn3_llm_multimodal_tensor tensor, int n_inputs, rknn_perf_metrics_t* perf)
{
    int ret;
    ...
    printf("--> inference gme_qwen_vl vision model\n");
    int start_us = getCurrentTimeUs();
    ret = inference_gme_qwen_vl_vision(&(app_ctx->vision), img, img_embeds);
    perf->vision_latency = getCurrentTimeUs() - start_us;
    if (ret != 0)
    {
        printf("inference gme_qwen_vl vision model fail! ret=%d\n", ret);
        return ret;
    }

    printf("--> inference gme_qwen_vl llm model\n");  
    ret = inference_gme_qwen_vl_llm(&(app_ctx->llm), tensor, n_inputs, perf);

    if (ret != 0)
    {
        printf("inference gme_qwen_vl llm model fail! ret=%d\n", ret);
        return ret;
    }
