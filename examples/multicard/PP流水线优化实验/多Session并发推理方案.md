...
6. **模型文件不需要重新转换。** 多会话是纯运行时改造，`.rknn`/`.weight`/`.safetensors`/`embed.bin`/`.tokenizer.gguf` 全部不动。这让版本管理简单很多（见 §8）。
   > **2026-09-20 边界**：这句对**用同一份导出**而言成立。**换上下文长度就是要重新导出**，
   > 而它会连带改掉每卡能跑几路（4096/8192 → 5 路，32768 → **2 路**，§9.17.1）——所以
   > "多会话改造不碰模型"和"上下文长度是容量的一边"这两件事**都要说、不冲突**。
7. **内存不是瓶颈，不需要重新导出模型。** 实测每会话 KV 仅 **64.4–70.7 MB/卡**，且**在 `rknn3_session_init` 时按满上下文一次性预分配**——填入 3894 token 上下文后新增分配为 **0.0 MB**。所以容量只与「会话数」有关，与实际用掉多少上下文**完全解耦**；4 会话总开销固定在 ~283 MB/卡（4×70.7），与上下文填满与否无关。
   ...
   >
   > ⚠️ **2026-09-20 补：上面这条只在 4096/8192 导出上成立。** 一旦换成大上下文导出，
   > **内存就自己变成瓶颈**：32768 那份导出每路 406.7 MB/卡，额度只够 **2 路**，
   > 此时"降 ctx 重导"正是**唯一的正解**。第 7 条那句"内存不是瓶颈、不需要重新导出"
   > 同理要带上"**在 4096 上下文下**"。容量 = **上下文 × 路数**，这是个乘积，
   > 不是单看哪一边（§9.17.1 / §9.17.2）。
...
14. **"NPU 空着，我却要等"是排队、不是算力（2026-09-18，§9.14）**：用户报的那次 `158 tok · 100.9s · 1.57 tok/s` 里，NPU 上一个请求都没在跑。会话池的回收只有一条阈值 `IDLE_TTL`（300s），**且只在 `acquire` 里发作**（"按需回收"，为的是不静默重算任何一段对话的 KV），两条合起来就是：N 段对话把 N 个会话**绑着不用**时，第 N+1 段要等到某一段静默满 300s 才拿得到会话。板上实测第 5 段**排队 226.0s**，它拿到会话的瞬间正是第一段跨过 300s 的那一刻——**等的是阈值，不是算力**。现在加第二层 `--contend-idle`（默认 15s）：**只有**一个空闲会话都找不到、这一轮非等不可时才降下来，且**只收最闲的那一段**（还有空闲会话时一次都不发作，常规路径逐字节不变；`idle_ttl=0` 仍是总开关）。板上 A/B（只换网关进程）：四段对话静默 70–90s 时第 5 段从 **排队 226.0s → 15.4s**（在静默 0.5s 时敲，`wait=15.0`）/ **0s**（在静默 >15s 时敲，`wait=0.0`、TTFT 1.17s，这是常见形态），`reaped_total` 一次只 +1。同时把"等"变成可见的：`X-KV-Reuse` 加 `wait=`、日志 `lease:` 行排过队就打 `排队 X.Xs`、`/v1/pool` 报 `contend_idle_s`、演示页行内显示等待态、统计行拆成"排队 / 实际推理"、有人排过队时汇总栏标"伸缩比不可引用"。**慢本身不可怕，不可归因的慢才可怕**——页面上那个 `1.57 tok/s` 把排队算进了分母，而屏幕上没有任何字段能把它和"模型慢"分开。

15. **演示页的 UI/UX 改版（2026-09-18，§9.15）**：顶部「待机」变成**带指示灯的徽标**（CSS 画的灯，不是 emoji——投影上 emoji 可能是个方框）、右侧控件改成一组 `flex/居中/gap=16px`、按钮分**主/次/危险**三级、数字输入框自绘边框+内阴影、**面板头右侧留出 TTFT + tok/s 的灰色小字**（底部那行是记账，看不见首字延迟）、面板内容加内边距、单行输入框换成**高度随内容自适应的 textarea**、发送键**内嵌**在输入框右下角。三处没照原话做：**不用 Tailwind CDN**（这一页是板卡离线发出来的，引 CDN 等于给"打不开"埋一个只在断网时发作的雷，要的是那套层级不是那个 CDN）、**灯用 CSS**、**面板头那格与底部统计行分工明确**。顺手堵了两个"屏幕上会骗人"的地方：徽标**必须同时换字和换色**（同一个灰灯下"生成中"和"待机"没区别，而灯的颜色恰恰是隔投影仪唯一看得出的东西）；**新一轮开始时那格必须清空**（挂着上一轮的首字延迟，在最该判断"到底开始了没有"的那一帧上给旧数字，比空着更坏——`page_check.js` 有一条断言守它）。回车分流：**Enter 发送、Shift+Enter 换行**（单行框里回车只有"发送"一个意思，换成 textarea 后回车默认换行，不拦就是"按回车没反应"）。验证分三层，各有边界：桩 DOM 的 `page_check.js`（**本机桩后端与真板卡各全绿一次**，板上 TTFT 1.0–1.7s、四路各 ~10 tok/s、聚合 35.73、会话全还干净）+ **无头浏览器截图**（真 CSS 排版：本机桩与真板卡各一张，抓到真机上**运行中**那一帧）+ 人眼。⚠️ 截图那层**没有断言**，"我看过了"不等于"以后不会坏"，真正守得住这次改动的只有那两条性能小字的断言。页面是现读现发的 ⇒ **只换文件、不重启网关**（省掉 240s 重载），旧件留 `serve/bak_20260918_ui/`。

16. **演示页"聚合 tok/s"两个口径混用，会把结论说反（2026-09-18，§9.16）**：用户真机上四路各自
    9.83/10.07/11.80/9.72 tok/s（**每一路都是全速**，单路基线约 10.6），横幅却写着"合计 2095 tok /
    墙钟 135.7s → 聚合 **15.43** tok/s"——跟单路是一个量级，"并发没用"就是这么读出来的。
    **根因是口径不是算错**：那 135.7s 里只有前约 18 秒是四路在跑（面板 3 生成 1602 token 跑了
    135.7s，另外三路约 16s 就收工），剩下约 120 秒**只有一路**在跑。于是
    **各路速率和** `Σ(tok_i/el_i)` = **41.42 tok/s**（四路同时跑的总产出，**与单路基线同口径**），
    **墙钟摊薄** `Σtok/墙钟` = 15.43 tok/s（分母里混进了三路空转的时间，**平均只有 1.37/4 路在跑**）
    ——**差 2.7 倍，是两个不同的问题**。改法：横幅主数改用**各路速率和**，**伸缩比只能用它**
    （同一轮用摊薄值会算成 1.45×，与真实的 3.89× 差到会改结论）；摊薄值照常显示，但长度不齐时
    横幅自己标出"平均只有 N/M 路在跑"，排队警告移到整句最后。顺带修掉一个**检查自己的盲区**：
    `page_check.js` 原先自己重发四路，**页面里 `summarize + setSum` 两句压根不执行**，汇总行怎么
    算的没有任何覆盖——现在检查直接调页面的 `runAll()`（用户点「②」那条路）并核对页面上真写出来的
    那句；另加一组**不联网的纯算术 fixture**（用的就是这次的真实数字），三个变异体全部被抓住。
...
>
> ⚠️ **2026-09-20 更正措辞**：上面那句里的"上下文长度"指的是**这一路实际用掉多少 token**，
> 不是**模型编进去的上下文长度**。**后者恰恰是容量的一边**：容量 = 上下文 × 路数。
> 这句话被引用了两轮都没带上这个限定词，最后落到"32768 的导出配 4 路"这个组合上，
> 算下来 406.7 × 4 = 1626.8 MB/卡，超了可给 KV 的额度 51% —— 代价是四张卡僵死、
> **只能重启板卡**（§9.17.2 完整复盘）。
...
> **2026-09-20 补：这个夹子是双向的。** 上面是"往上传不上去"，还有反过来的那一半——
> **32768 那份导出传 `--ctx-size 8192` 仍然挑 32768**（一份导出只有一档 KV：`n_lens=1`）：
>
> ```
> requested max_context_len=8192 → chosen kvcache_buffer_lens: 32768
> ```
>
> **⇒ 拿一份大上下文的导出想靠 `--ctx-size` 省内存是省不掉的，只能换一份更小的导出。**
> 而 `--ctx-size` 同时**不是** `max_ctx_len` 的来源（那个来自
> `rknn3_query(RKNN3_QUERY_LLM_CONFIG)`，是编译进模型的），所以两个方向都不能靠它救。
> 权威答案永远只有一处：SDK 日志里的 `chosen kvcache_buffer_lens`（§9.17.4 给了不启模型
> 就看出来的办法：量 rope 外置张量）。**顺带一提，"一份导出一档 KV"这件事本身就是
> §9.17.1 那个容量的由来**。
...
2. **调小 `kvcache_len` / `--ctx-size` 重新导出，无法提高并发数。** 本文档早期版本把「降 ctx 重导」当作容量不足时的退路——**该退路已作废**：它降低的是每会话内存占用，而限制并发的不是内存。这个方向的唯一价值是省内存（若将来需要给别的功能腾空间），不是提高并发。
   ...
   > ⚠️ **2026-09-20 加限定：这条推论只在 KV 不是瓶颈时成立。** 上面那句"限制并发的不是内存"
   > 是**在 4096 导出上量出来的**（第 6 个会话失败时最紧 node 还剩 93.7 MB）。**换到大上下文
   > 导出时，KV 会自己变成瓶颈**：32768 那份导出每路 KV **406.7 MB/卡**，
   > 1078 MB/卡 的额度只够 **2 路**（实测，§9.17.1）——此时**"降 ctx 重导"恰恰是唯一的正解**。
   > 更要紧的是**别把 §4.1 那句"容量与上下文长度完全解耦"读成"上下文不影响容量"**：
   > 它说的是"这一路**实际用掉**多少 token 不影响占用"（KV 按满上下文预分配），
   > 而容量 = **上下文 × 路数**，上下文就在乘积里（§9.17.2）。
...
### 9.11 帧上限那条分支的 `break`：一个 3.4 MB 的请求把整套服务打死 —— 2026-09-17 修复并上板验收

§9.10 刚把「上下文装不下」那条判据补好，当天用户就用一个**真正的大请求**把板卡打死了。
网关日志的现场：`243 messages, 3492840 prompt bytes, stream=True, max_new=64000, tools=18`，
一条 `POST /v1/chat/completions` 拿到 **503**，接着四段对话（4 个终端 + 1 个 Claude Code）
一段接一段进队列，队列从 1 人涨到 5 人。

#### 9.11.1 根因：一条请求 → 一个进程

链条有六步，每一步单独看都不像故障：

1. 渲染后的 prompt **3492840 B** 越过了 `main.cc` 的 `kServeMaxPromptBytes`（1 MiB）；
2. 输入线程那条分支当时写的是 `serve_err(...)` 之后 **`break`** —— 跳出输入循环；
3. 输入线程退出 ⇒ `input_eof` ⇒ **整个后端进程收工退出**。它退出时打印的是平时那份
   *正常的收尾摘要*，所以**日志里一个字都看不出错**（这条是这次最贵的一课：错误信息
   发在帧通道上，而进程的收尾输出在 stdout）；
4. 网关这边 `stdin.write` 拿到 `BrokenPipeError` ⇒ `BackendError("failed to write
   request: ...")` ⇒ HTTP **503**；
5. 那一轮 `q is None`，网关照旧 `mark_dead(lease.session)`；之后每个请求都租一个会话、
   都以同样的方式失败 —— **四个会话全 dead**，而 `dead[]` **没有复活路径**；
6. `acquire()` 那时还从不问后端死活，于是后来的每个请求都在这里排满 `queue_timeout`
   （600s）；**而 `/health` 还在报 `sessions: 4`**。

> **`当前 0/4 被占` 且队列在涨，只是一个"没人在跑活"的信号，不是定性。** 它有两种可能：
> 四段对话都静默在 `IDLE_TTL` 窗口里（正常，最多 300s 自己会好），或者四个会话全死了
> （故障，等到天亮也不会好）。**分辨只能靠 `GET /v1/pool`**：现场快照里是 4×`dead` + 5 个
> waiting，这才是定性。`/health` 那一行当时什么也证明不了。

#### 9.11.2 为什么 §9.10 的 REJECT 没拦住它

因为撞的不是同一条判据。`kServeMaxPromptBytes` **不是"这一轮太长"的判据**，它是"别相信
头行里那个数字"的**内存护栏**：`prompt_len` 是帧头里一个客户端可写的整数，不设上限的话
一个手滑（或恶意）的 `2^60` 就能让进程按那个数字去 reserve 内存。它 1 MiB 的取值远在
任何 8192 位置装得下的 prompt（几十 KB 量级）**之上**，所以**在这里被拦下的请求，tokenizer
那条也一定拦得住**——两条判据的**适用范围重叠，区别只在拦下之后干什么**，而这里曾经
写的是 `break`。

#### 9.11.3 怎么修

**服务侧**（`main.cc`）——把载荷丢掉接着跑，回一帧 **REJECT**：

```cpp
if (prompt_len > kServeMaxPromptBytes) {
  discard_serve_payload(prompt_len);   // 读掉它但绝不 reserve：撒谎的头行也吃得住
  serve_reject(rid, session,
               "prompt too long: " + std::to_string(prompt_len) +
                   " bytes exceeds the frame limit of " +
                   std::to_string(kServeMaxPromptBytes) + " bytes; shorten this request");
  continue;                            // 不是 break：一条请求发错了，不该升级成服务没了
}
```

**回 REJECT 不回 ERR**，因为会话是好的、这一轮**一个 token 都没跑**。也**不发 CLEAR**：
KV 一个字节都没动，网关那边的 `known[]` 保持原样正好描述着 KV 里真正装着的东西
（§9.10 那一支发 CLEAR 是因为它确实动过 KV）。

**网关侧**（三条 containment，都是"别让人在队列里空等"）：
`acquire()` 在**进队列之前**先判后端死活与"四个会话全 dead"，两条都当场报错而不是排队；
`/health` 不再谎报（新增 `backend_alive` / `sessions_dead`，`status` 分
`ok` / `degraded` / `backend_exited`）；帧读线程在流结束时**大声打印一行**（自己发 `QUIT`
时用 `stopping` 抑制，否则正常关停也会鬼叫）。

#### 9.11.4 板上 A/B：同一个探针、同一个网关，只换后端二进制

对照件是上一版后端 `b6a67b2d`——它**已经认得 REJECT 帧**（§9.10 那一版），两版唯一的差别
就在载荷超限这条分支，所以这是一次干净的对照。探针 `oversize_probe.py` 三步走：
基线小请求 → 一条 1.2 MB 的请求 → 紧随其后的普通请求 + `/health` + `/v1/pool`。

| 步骤 | 改动前 `b6a67b2d` | 改动后 `6a050c90` |
|---|---|---|
| 基线小请求 | 200 | 200 |
| 1.2 MB 请求 | **503** `failed to write request: BrokenPipeError(32, 'Broken pipe')` | **400** `prompt too long: 1228948 bytes exceeds the frame limit of 1048576 bytes; shorten this request`（**0.1s**） |
| 紧随其后的普通请求 | **503** `backend exited; every request fails until the gateway is restarted` | **200** |
| `/health` | `status: backend_exited, backend_alive: false, sessions_dead: 1` | `status: ok, backend_alive: true, sessions_dead: 0, sessions: 4` |
| `/v1/pool` | `['idle','dead','free','free']` | `['idle','idle','idle','free']`（收尾全 `free`） |

第 3 行值得单说：改动前那台跑的是**新网关**，所以它当场回的是"后端已退出"；**现场那台是
旧网关**，它做不到这件事，只能把人放进队列等满 600s——用户日志里那 5 个排队的人走的就是
这条路。两处都是本轮修的，但**不是同一处**：一个在服务侧，一个在网关侧。也就因此，
A/B 表里"改动前"那一列**不能**当成现场症状的复刻。

> 0.1 秒这个数不是装饰：载荷是**丢掉**的，一个 token 都没进 tokenizer。对比 §9.10 那条
> tokenizer 判据——它必须把整段 prompt 数一遍才能下结论。

**完整探针**（`verify_reject_fix.py`，现在两条 REJECT 路径都打）在修好的板上 **14/14 PASS**：
4 条 15719-token 超限请求全部 400、零 dead、可用额度恒为 4、小请求 200；**>1 MiB 那条 400，
且同一段对话接着问还是 200**；6 个探针会话 `close` 全 200。

**并发路径也确实回来了**（`http_scaling.py`，96 token/路，修复后的连量）：

| N | 墙钟(s) | 聚合 tok/s | 伸缩比 | 用到的会话 |
|---|---|---|---|---|
| 1 | 8.9 | **10.84** | 1.00x | `[0]` |
| 2 | 9.5 | **20.15** | 1.86x | `[1, 0]` |
| 4 | 10.5 | **36.40** | **3.36x** | `[0, 1, 3, 2]` |

与 §9.10.8 的 10.93 / 20.09(1.84x) / 37.22(3.40x) 在轮间波动范围内；重叠断言
（墙钟 ÷ 各路耗时之和）0.50 与 0.25，都 < 0.75，说明量到的是**并发**而不是排队。

#### 9.11.5 主机测试台：t3 换了判据，新增变异 m6

`t3` 原来钉的是"头行里写个 2^60 ⇒ 回 ERR、输入线程收工"——**那正是打死服务的那条行为**。
它不是"断言写松了"，是**把错的行为钉成了对的**，所以它在测试台里绿了很久也不能算数。
改后的判据钉两件事：超限那轮**只回一帧 REJECT**（并说清是本轮长度），**紧接着那条正常
请求照常进门且载荷完好**——后半句才是关键，它证明输入线程还在跑。t3 因此从对照组搬进
"改动前必须失败"组（对照组只剩 t4 / t9 / t10）。

新增变异 **m6** = 把新分支换回 `serve_err` + `break`，只让 t3 变红：

```
mut_oversized_kills t3: 如预期地失败（2 条）
  [FAIL] 超长那轮只回一帧 REJECT（实际: ERR(700)）
  [FAIL] 紧跟着的那条照常被收下、载荷完好（实际 0 条 / "-"）
```

> 夹具自己也踩了一个坑：1 MiB 的载荷**不能**从 64 KiB 的管道喂给被测进程——写侧会阻塞，
> 而看门狗把它报成"测试挂死"（`rc=99`）。**那是夹具的错，不是代码的错**，却会被记成一次
> harness 失败，而且看起来像被测代码有问题。所以新增了 `open_stdin_file()`：大载荷走
> 临时文件 + `dup2` 到 fd 0。

工作区 11 个测试 **72 PASS / 0 FAIL**；钉死的改动前提交 `8792b01` 上 t1/t2/t3/t5/t6/t7/t8/t11
全红、t4/t9/t10 仍绿（对照组）；6 个变异体各自让对应测试变红。网关自检 **79 PASS / 0 FAIL**，
其中新增的两条正是这次的 containment：

```
[PASS] 后端没了 => 立刻报错，不在队列里空等到 queue_timeout 0.00s
       BackendError('backend exited; every request fails until the gateway is restarted')
[PASS] 四个会话全 dead => 立刻报错 …… BackendError('all 4 sessions are dead; restart the gateway')
```

#### 9.11.6 工序上又一条会**静默失败**的事：覆盖正在运行的二进制

`cp` 一个正在跑的二进制会得到 `cp: cannot create regular file ...: Text file busy`
（ETXTBSY）。所以"装上新二进制再重启"这个顺序必须拆成**先停网关、再拷、再起**；先拷后停
的话**拷这一步就失败**，而如果脚本没 `set -e`，后面照样打印"重启成功"。本轮做 A/B 时
就是这么撞上的。
...
| 件 | md5 |
|---|---|
| `examples/multicard/cpp/main.cc`（源码，本地 == 板上 `native_build/main.cc`） | `d7d7f95e28fffa93831a60ead704bd2a` |
| `examples/multicard/serve/rkllm_gateway.py`（本地 == 板上） | `cf5728e2b2615bf1174ebdecf7fba4f7` |
| 后端二进制（板上 `rknn_multicard_demo.serve`，**板上原生编译**） | `6a050c908c93619a245c071336e1ec4d` |
| 后端二进制（上一版 §9.10，A/B 的对照件）—— 留在板上做回滚 | `b6a67b2d1ede1d8e0cb2a63db6830e3e` |
| 网关（上一版，改动前）—— 留在板上做回滚 | `166d1543000f764b505d04eb40b72fba` |

启动参数与 §9.10.8 逐字一致（`CTX=8192 --core-mask 0xff --stage-count 4 --bucket-size 128
--sessions 4 -n 512`）。

#### 未验证 / 已知限制

- **A/B 里"改动前"那一列跑的是新网关**，所以它复现的是"后端退出后请求立刻 503"，而不是
  现场的"排满 600s 才 503"。"旧网关 + 旧后端"这个组合**没有复跑**（要跑得把网关也换回去，
  而现场症状已有用户侧的日志作为证据）。
- **1 MiB 那条判据看不到它"单独生效"的样子**：被它拦下的请求，tokenizer 那条本来也会拦，
  区别只是代价与报文。**换更长的上下文导出时，这个 1 MiB 要重新审**。
- 4 路并发这一轮量的是 **HTTP 层**（`http_scaling.py`）。用户的 msclient 四终端
  （带 rendezvous 改动的那份）**这一轮没有重跑**。
- **把 Claude Code 接到这块板上时别把它留在后台**：它的请求（几万 token、几 MB）现在
  只会拿到一个干净的 400，但**照样会占住一个会话到 `IDLE_TTL`**，吞吐测试的数字会被它拖低。
...
### 9.14 NPU 空着却在排队：会话池加一层「有人在等就收最闲的那一段」（2026-09-18 修复并上板验收）

**症状（用户报的）**：`明明 NPU 现在没人使用，但是我输入问题后还是在等待`。网页某格的统计行是
`158 tok · 100.9s · 1.57 tok/s · 会话 2 · KV 213/213（全量）`。

**先把"慢"拆开。** 网关上那套每 token 的时刻戳能一眼分开两种慢：慢在**解码**会抬中位间隔，慢在
**排队**只会抬首 token 时间、间隔一动不动。板上四路并发（`page_repro.py`，与网页同形态）量到
TTFT 1.09–1.75s、中位间隔 0.084–0.092s（= 10.9–11.9 tok/s/路，与 §1.1 的单会话基线吻合），
**解码侧一点没坏**。坏的是另一边：网关日志里有一次 `等了 138.1s`，pool 快照显示那段时间**四个
会话全是 `idle`** —— 没有任何人在占用 NPU。

**根因**：会话池的回收只有一个阈值 `IDLE_TTL`（默认 300s），而 `_reap_idle_locked` **只在
`acquire` 里被调用**（这是"按需回收"的刻意设计：不静默重算任何一段对话的 KV）。两条合起来就
成了：N 段对话把 N 个会话**绑着不用**时，第 N+1 段对话要一直等到某一段静默满 300s 才拿得到
会话，而这 300s 里 NPU 全空。板上实测：第 5 段**排队 226.0s**，它拿到会话的瞬间正是 `web-1`
跨过 300s 的那一刻 —— 等的是**阈值**，不是算力。

`1.57 tok/s` 是**端到端**的数：页面的计时器从 `fetch` 之前起算，而 SSE 的响应头是在**拿到租约
之后**才 flush 的，所以这段排队全被算进分母。屏幕上没有任何字段能把它和"模型慢"分开 ——
这才是这条报到用户面前的原因（**慢本身不可怕，不可归因的慢才可怕**）。

**修法：两层阈值，第二层只在第一层不够用时降下来。**

| 层 | 阈值 | 触发条件 | 收几段 |
|---|---|---|---|
| 一（原有） | `IDLE_TTL`，300s | 有人问就一直在跑 | 所有静默超阈值的 |
| 二（新，`--contend-idle`） | 15s | **只有**一个空闲会话都找不到、这一轮非等不可 | **只收最闲的那一段** |

第二层的三条自我约束：**①** 只在"本来就要等"时下降 —— 还有空闲会话时它一次都不发作，常规路径
逐字节不变；**②** 只收**一段** —— 收多了会把 KV 复用成片打掉，而排队的人是**一个**；
**③** 收的是**最闲**的那一段（按静默时间倒序取第一个非 busy 的）。`idle_ttl=0` 仍是**总开关**：
第二层绕开它就等于把"0 = 不回收"这条契约作废，而回归套件正是靠它来证明自己真的在主动
`close`（不是碰巧被回收救了）—— 这条在代码里写了注释钉住。

**顺带把"等"变成可见的**（不然下次还是只能靠 pool 时间线去猜）：SSE 响应头 `X-KV-Reuse` 加
`wait=`、网关日志的 `lease:` 行排过队就打 `排队 X.Xs`、`/v1/pool` 报 `contend_idle_s`、演示页在
排队期间行内显示 `⏳ 等待空闲会话…`、统计行拆成 `（其中排队 X.Xs，实际推理 Y.Ys）`、四格汇总栏
在有人排过队时标一句"伸缩比不可引用"（排队会同时污染 token 数与墙钟，那个比值就失去意义了）。

**板上 A/B（同一块板、同一份模型，只换网关进程）**：

| | 四段对话的静默 | 第 5 段对话 | `reaped_total` |
|---|---|---|---|
| 改前 | 70–90s | **排队 226.0s**（等满 `IDLE_TTL`） | — |
| 改后，在静默 **0.5s** 时敲 | 0.5s | 进队列，**15.4s** 拿到会话，`wait=15.0` | 0 → 1 |
| 改后，在静默 **24–42s** 时敲 | >15s | **没进队列**，`wait=0.0`、TTFT 1.17s | 1 → 2 |

第二行才是这一层的正名：**排队仍然存在（该等就等），但上界从 `IDLE_TTL` 降到了
`contend_idle`**；第三行是常见形态（网页四格搁置一两分钟后再问），直接命中。另注意 `reaped_total`
一次只 +1，且静默已 41.8s 的两段**仍不被收** —— 后台不会偷收，只在真有人敲门时才动。
日志两行：`[pool] 会话回收：id:web-3 静默（有人排队） 73.0s > 15.0s，s3 交还给排队者`、
...
| 项 | 结果 |
|---|---|
| 网关 `--selftest`（桩后端） | 全绿，含新增的"有人在等时才收最闲的一段"7 条 |
| `serve_http_test.py`（桩后端） | 全绿 ×2，含新的 `/v1/pool` 报 `contend_idle_s` |
| `page_check.js`（桩后端） | 全绿，含新的等待态与排队拆解两条断言 |
| `run_all_board_tests.sh`（真板卡） | 8 段 rc=0、**FAIL 0 / 非零 rc 0 / traceback 0**；伸缩 **10.92 / 20.27(1.86×) / 36.80(3.37×)** tok/s |
| `page_check.js` 对真板卡 + 真模型 | 全绿，四路落四个会话、聚合 36.13 tok/s、关页面与清空都把会话还干净 |
| 常规路径未受影响 | 冷启四路 TTFT 0.89–1.55s、中位间隔 0.086–0.092s、`wait=0.0`、`reaped_total` 全程 0 |

**未验证 / 已知限制**：`contend_idle` 的取值（15s）是**拍的**，没有扫过参数空间 —— 它的意义是
"比一次生成的耗时长一点、比一个人的耐心短得多"，往下压会把正在思考的对话的 KV 打掉（代价是
下一次全量 prefill），往上抬就是回到这次修掉的那个症状。**第二层只对"按需回收"负责，不是
调度器**：真正忙满时它一秒都不管用（那时该排队），多人场景下 `R12`（无鉴权 ⇒ 一个客户端能拿
多份身份占满会话）也还在。

**工序**：改的是网关进程 → 必须重启（模型重载 ~240s，页面上已有的会话会断）；旧件留
`serve/bak_20260918_contend/`。五个文件下发前后 md5 逐一核对一致。启动脚本新增 `CONTEND_IDLE`
环境变量（默认 15）；⚠️ **`CONTEND_IDLE=0` 就是关掉这一级**（退回"永远等到 `IDLE_TTL`"），
不是"零秒回收"。变更台账见 `serve/CHANGELOG.md`。

---

### 9.15 演示页的 UI/UX 改版（2026-09-18）

用户点名的几条：顶部「待机」改成带指示灯的徽标、右侧控件对齐、按钮分三级、
输出上限输入框美化、面板头右侧留出 TPS/Latency 的位置、面板内容加内边距、
输入框改多行自适应高度、发送键与输入框对齐。**都不是功能缺陷，逐条落在排版上**——
所以这一节的判据不是"跑绿了"，是**屏幕上看得对**。

#### 9.15.1 三处不是照抄用户原话的地方

1. **没有用 Tailwind CDN，继续用页内那套 CSS 变量。** 这一页是**板卡离线发出来的**
   （`_static_demo`），演示现场未必有外网：引一个 CDN 等于给"打不开页面"埋一个
   只有断网时才会发作的雷。要的是 Tailwind 那套层级（primary/secondary/danger、
   `flex items-center gap-4`），不是那个 CDN —— 所以按同样的意图写成了
   `.btn-primary` / `.btn-secondary` / `.btn-danger` 与 `.controls{margin-left:auto;gap:16px}`。
2. **「待机」徽标里的灯是 CSS，不是 emoji。** 用户给的例子是 🟢/🟡，但 emoji 在
   Windows / 安卓 / iOS 上长得不一样，投影出来还可能是个方框；而这块牌子恰恰是要
   **隔着投影仪一眼看出板卡在不在跑**。改成 `<i class="led">` + `.badge.<state>`
   （`idle` 绿 / `busy` 琥珀闪 / `wait` 蓝闪 / `err` 红），JS 换的是 **class 和字两样**。
3. **面板头那格和底部统计行不是重复。** 用户要的是"TPS + Latency 的灰色小字"，
   而底部那行是**记账**（token 数、排队拆解、会话、KV）。所以上面那格只放两个数：
   **首字延迟（TTFT）+ tok/s** —— 底部那行**看不见 TTFT**，而"要等多久才出第一个字"
   正是给人看并发最直观的一个数。

#### 9.15.2 顺手补的两处"屏幕上会骗人"

- **徽标不能只改字不改色**：同一个灰灯下"生成中"和"待机"没有区别，那这个徽标就白加了。
  `setStatus()` 一个字面量一个字面量地 `classList.remove`（**不能写成
  `remove("idle","busy","wait","err")`**：页面的桩 DOM 里 `remove` 只认第一个参数，
  多传的会被静默丢掉 —— 桩里绿、浏览器里绿、真跑起来是花的）。
- **新一轮开始时，面板头那格必须清空**（`p.ttft = null; setPerf(p, "")`）：留着上一轮的
  首字延迟/速率，在最该判断"到底开始了没有"的那一帧上给的是旧数字，**比空着更坏**。
  `page_check.js` 专门有一条断言守这个。

#### 9.15.3 输入区：多行 + 内嵌发送键

`<input>` 换成 `<textarea rows="1">`，`grow()` 按 `scrollHeight` 长高、夹在 120px 以内
（超过就内部滚动）。发送键**内嵌在框的右下角**（`position:absolute`）而不是并排：
框长高时并排的按钮要跟着重新对齐，嵌进去就永远贴着那一角。
回车分流也跟着改：**Enter 发送、Shift+Enter 换行** —— 单行框里回车只有"发送"一个意思，
换成 textarea 之后回车默认是换行，**不拦的话按回车什么都不发生**。

`grow()` 第一句是 `if (typeof el.scrollHeight !== "number") return;`：桩 DOM 没有排版、
没有 `scrollHeight`，不挡的话那边会往 `style.height` 里写 `"NaNpx"` —— 不报错、也没意义。

#### 9.15.4 怎么验的：三层，各有各的边界

| 层 | 验什么 | 验不到什么 |
|---|---|---|
| `node page_check.js <桩网关>` | 页面**真实 JS** 的契约：身份、四个会话、交还、性能小字写出来了、新一轮那格是空的 | 排版（桩没有 CSS，也没有 `scrollHeight`） |
| 无头浏览器截图（本机 Chrome `--headless=new`，**一次性探针不入库**） | 真 CSS 排版：徽标配色、按钮三级、四行自适应输入框、内嵌发送键、四格同时激活那一帧 | 不是自动断言，只是"看过了" |
| `node page_check.js http://<板卡IP>:18280` | 真板卡 + 真模型端到端 | 同上 |

**探针那一步是本节的临时工序**，不进仓库：把页面复制一份，尾部接一段**调用页面自己的
函数**（`setStatus` / `setSum` / `setPerf` / `grow` / `render`）把各种状态灌进去，
真机那版则只把 `API` 指向板卡、加载后自动点一次「②」，于是抓到的正是**运行中**那一帧
（四格蓝框、琥珀徽标、性能小字空着、输入框已缩回一行）。

顺带记一个坑：探针里那个多行输入框的文本，我用 Python heredoc 拼的时候 `\\n`
没活下来、变成了**真的换行**，于是 JS 字符串字面量断行 —— **整段探针语法错、一声不响地
不执行**，而截图看起来只是"页面没变化"。`--dump-dom | grep` 也骗了我一次：
那些字符串**在脚本源码里也在**，grep 命中的是源码不是渲染结果；
**判据要看只有运行时才会出现的属性**（`class="badge busy"`、`data-probe-err`）。

#### 9.15.5 上板与验证

页面是**现读现发的**（`_static_demo` 每次请求读磁盘），所以**只换文件、不重启网关**
（省掉 240s 的模型重载）。旧件留 `serve/bak_20260918_ui/`，两个文件（页面 + 检查夹具）
下发前后 md5 逐一核对一致。

| 项 | 结果 |
|---|---|
| `page_check.js`（本机桩后端，`--idle-ttl 0`） | **全部通过**，含新增两条断言 |
| 无头浏览器截图（本机桩 + 真板卡各一次） | 徽标配色/按钮三级/四行输入框/内嵌发送键/四格同跑一帧，逐张看过 |
| `page_check.js` 对真板卡 + 真模型 | **全部通过**：四路 `web-1..4`、四个不同会话、TTFT **1.0–1.7s**、各路 ~10 tok/s、聚合 35.73、关页面与「清空」都把会话还干净 |

**未验证 / 已知限制**：① 截图这一层**没有断言**，"我看过了"不等于"以后不会坏"——
真正会守住这次改动的只有 `page_check.js` 里那两条性能小字的断言（元素还在不在、是不是空），
配色与间距仍然只能靠人眼；② **没有在触摸屏/窄窗口上试过**：`.controls` 排不下时会换行，
没量过 1024 宽以下的样子；③ 面板头那格在**流还没开始时是空的**（这是有意的，见 §9.15.2），
第一次看到的人可能会以为它没生效。

---

### 9.16 演示页"聚合 tok/s"的口径：两个"平均"混用会把结论说反（2026-09-18）

#### 9.16.1 症状：四路各自全速，横幅上的数却像单路

用户真机上点完「②」后，页面顶部写着（下方是四个面板各自的统计行）：

```
4 路并发：合计 2095 tok / 墙钟 135.7s → 聚合 15.43 tok/s

面板 1  9.83 tok/s · 153 tok · 15.6s      面板 3  11.80 tok/s · 1602 tok · 135.7s
面板 2 10.07 tok/s · 182 tok · 18.1s      面板 4   9.72 tok/s · 158 tok · 16.3s
```

四个面板**每一路都是全速**（单路基线约 10.6 tok/s），但顶上那个 `15.43` 与单路是同一个
量级，"四路并发没什么用"就是这么读出来的。

#### 9.16.2 根因是口径，不是算错

那 135.7s 里**只有前约 18 秒是四路在跑**：面板 3 生成 1602 token、跑了 135.7s，另外三路
约 16s 就收工了，剩下约 120 秒**只有一路**在跑。两个"平均"在这一轮里差了 2.7 倍：

| 量 | 公式 | 这一轮的值 | 它答的是什么问题 |
|---|---|---|---|
| **各路速率和** | `Σ(tok_i / el_i)` | **41.42 tok/s** | 四路同时跑时，整机一共在吐多少 token |
| **墙钟摊薄** | `Σtok_i / 墙钟` | 15.43 tok/s | 把总产出摊到整段墙上 —— 分母里**混进了三路空转的时间** |
| 时间平均在跑路数 | `Σel_i / 墙钟` | 1.37 / 4 | 上面那个数为什么小：平均只有 1.37 路在跑 |

所以 `15.43` 不是"并发性能退化"，它量的是"平均有几路在跑"。四路**长度齐**的时候两个数
相等（`Σel ≈ n·墙钟`），一旦参差就分道扬镳——而演示时四路长度恰恰经常差得多。

#### 9.16.3 改法：主数用速率和，摊薄值戴着帽子出现

`demo_4chat.html` 里抽出纯函数 `summarize(rs, wall, baseline)`：

- 横幅**主数** = 各路速率和（`<b>` 加粗），后面括号里跟合计 token 与墙钟；
- **伸缩比只用速率和 ÷ 基线**，因为基线的口径就是"某一条路自己的速率"，两者同口径。
  同一轮里用摊薄值算会得到 `15.44/10.63 = 1.45×`，与 `41.36/10.63 = 3.89×` 差到会改结论；
- 摊薄值**照常显示**（四路长度齐时它才是那个有用的数），但 `Σel/墙钟 < n−0.5` 时横幅自己
  标名字：`四路长短不齐，平均只有 1.37/4 路在跑，墙钟摊薄 15.44 tok/s（空转的路也算进了
  分母，别当并发性能看）`；
- 排队警告（§9.14）移到**整句最后**——它撤的是前面每一个数，最扎眼的是刚印出来的伸缩比。

阈值 `n−0.5` 的取法：`Σel/墙钟` 的物理上界是 `n`（四路跑满全程），四路等长时它略小于 `n`；
0.5 的余量留给正常抖动（约 ±6%），够松到不误报、够紧到"一路早收工"必然触发。

#### 9.16.4 检查自己的盲区：汇总行以前**没有任何覆盖**

`page_check.js` 原先自己 `panels.map(send)` 发四路，**页面里 `summarize + setSum` 那两句
压根不执行**——于是"汇总行怎么算的"改坏了也全绿（这正是这次口径问题能活到现在的原因之一）。
现在把「②」的处理器提出来成 `runAll()`，检查直接调它、用 `lastRound()` 取回这一轮的结果，
再核对**页面上真写出来的那一句**（只比"各路速率和"这一项，不比整句：两边墙钟分别来自
`performance.now()` 与 `Date.now()`，拿整句比等于在测时钟抖动）。

另加一组**不联网的纯算术 fixture**，用的就是上面那次真实运行的数：长短不齐（153/182/1602/158）、
四路等长（4×150/15）、某路排队三种。三个变异体都被抓住：

| 变异 | 预期 | 实际 |
|---|---|---|
| 伸缩比改用摊薄值 | 报 1.45×、FAIL | ✅ 3 条 FAIL（含排队那条） |
| 稀释警告永不触发 | 横幅不再自证，FAIL | ✅ 2 条 FAIL |
| 把摊薄当主数印（只改字面） | 抓不到就说明断言只认位置 | ✅ 2 条 FAIL |

（三个变异体都**在联网那部分之前**就 `exit`，所以试一次只要几秒；这也是"夹具放在前面"的用意。）

#### 9.16.5 验证与边界

| 项 | 结果 |
|---|---|
| `page_check.js` 本机桩后端（`--idle-ttl 0`） | **全部通过**（含 3 组 fixture 与"页面写的 = 独立算的"） |
| 三个变异体 | 全部 FAIL，且失败在联网之前 |
| 无头浏览器截图（不联网，直接喂真实数字） | 最长形态（稀释 + 排队 + 伸缩）在 1280 宽下**换两行**，读数正常 |
| 真板卡 | **本次未做**：下发时板上网关没在跑（`:8080` 拒连、NPU 代理在但是网关进程不在），见下 |

⚠️ **页面是现读现发的**，所以这次仍是"只换文件、不重启网关"（§9.15.5）。旧件留
`serve/bak_20260918_sum/`，两个文件下发前后 md5 逐一核对一致。

**未验证**：① 真板卡上"四路真的参差"的一次跑（板上网关当时没起，没为了这一条去重启一个
要 240s 加载模型的服务）；② fixture 里的四个数是**从用户屏幕抄下来的**，不是板上重跑的，
所以钉住的是"公式对这几个数给出什么"，不是"板上一定会出现这几个数"；③ 阈值 `n−0.5` 只在
桩上试过正常抖动，真机上的抖动幅度没量过。

---

### 9.17 32K 上下文：**会话数由那份导出决定**，页面窗口数不能再写死（2026-09-20）

用户的一句话把两件事绑在了一起：

> 32K 上下文不强求 4 session 对话，你测试出最多能接受几个 session 并发，然后我想实现网页端
> 32K 上下文的 chatbot。网页端也需要改变……**可以根据上下文的长度选择窗口 session 的个数，
> 可以增添删除窗口，但是不能超过上下文限制的 session**

拆成两问：**① 这份 32K 导出最多几路？② 页面怎么让窗口数服从这个数？** 第 ② 问的前提是
第 ① 问有答案——而第 ① 问的答案不在代码里，在**模型文件的大小**里。

#### 9.17.1 先量：32K 导出每卡只够 **2** 路

上一轮把 4096 导出换成 32768 导出、按 `CTX=32768` 起，单会话就卡死过整套服务。这次按
"每次只多加一路、每加一路先看 smi 再决定下一步"的走法量，得到的内存账：

| 量 | 值 |
|---|---|
| 每卡 KV 总量 | 5071 MB |
| 权重 + internal（已占） | 3821.7 MB（卡 0–2）/ **3993.4 MB（卡 3）** |
| 可给 KV 的额度 | ≈ **1078 MB/卡** |
| **每路 KV @32768** | **50.8 MB/node = 406.7 MB/卡** |
| 1 路 | smi 84% / 88%，最紧 node 剩 **62.7 MB** |
| 2 路 | smi 93% / 95%，最紧 node 剩 **12.6 MB**（建第 2 个会话时实测 62.7 → 12.6） |
| 3 路 | 需要在最紧 node 上再挤 50.8 MB，而那里只剩 12.6 ⇒ **不可能** |

> **结论：这份 32768 导出每卡天花板 = 2 路会话。** 对照 P0 时代 4096/8192 导出实测的 5 路
> （§4.1/§4.4），**同一个 5 卡上限并没有变，是"上下文 × 路数"这个乘积顶到了墙上**。

反推出的单价是 **12.4 KB/token/卡**（50.8 ÷ 32768 × 4 node ≈ 0.0124 MB/token/卡），它能同时
对上三个实测点：4096 → 50.8 MB/路、8192 → 101.7、32768 → 406.7，而 1078 ÷ 406.7 = 2。
这条线性式就是下面那道闸的全部算术。

⚠️ **别把 §4.1 那个 8.05 MB/node/session 拿来一起算**：那是 `node_mem_info` 的增量口径
（预分配只认 node 池），和这里"卡上总量 − 权重 − internal"的账**不是同一把尺子**
（同一个 4096 在两把尺子下分别算出 32.2 与 50.8 MB/卡）。**跨口径相除会得到一个看着很专业的
假常数**——这次差点就这么干了。

#### 9.17.2 一个口径上的纠正：**§4.4 那句推论有适用条件**

§4.4 的结论是"**5 这个上限不由内存决定**"，并推出「调小 `kvcache_len` 重导无法提高并发数」。
在 **4096/8192 导出上它是对的**（第 6 个会话失败时最紧 node 还剩 93.7 MB）。但本文档
§4.1/§10 里那句"容量只与**会话数**有关，与上下文长度**完全解耦**"是**给读歪了**的：

- 它原本说的是"**这一路实际用掉多少 token** 不影响占用"（KV 在 `session_init` 时按满上下文
  一次性预分配，填满 3894 token 后新增分配 0.0 MB）——**这句仍然成立**。
- 但它**不**意味着"上下文长度不影响容量"。容量 = **上下文 × 路数**，上下文进了乘积的一边。
- 于是 §4.4 的推论要加前置条件：**只在 KV 不是瓶颈时成立**。32K 下 KV 就是瓶颈
  （406.7 × 3 > 1078），此时"降 ctx 重导"**恰恰是唯一的正解**——4096 那份导出每卡能跑 5 路，
  32768 这份只能跑 2 路。

**这正是 2026-09-20 那次撞穿的认识论根源**：一句"与上下文长度解耦"被两处引用、都没带上
它原本的限定词，最后落到"32768 的导出配 4 路"这个组合上——而那个组合在算术上是 406.7 × 4
= 1626.8 MB/卡，比可给 KV 的额度超了 51%。

#### 9.17.3 撞穿之后长什么样：**不报错，把四张卡焊死**

这一条必须记全，因为它的代价是**重启板卡**：

```
rknn3_session_init 分配失败 → 卡在 wait_event(0x100000): timed out
  → recv MsgSessionInitAck fail!          ← 那个端点从此不再应答
  → 之后每次加载都在 rknn3_init 上 ERROR_PIPE
  → systemctl restart rknn3 救不回来       ← 四个 rknn3_transfer_proxy 一起没了
                                            （ddr load addr request failed / 退出码 234）
  ⇒ 只能重启板卡
```

**分配失败不报错、反而把设备挂死，是官方闭源那半截的行为**——我们这侧能做的只有**别让它
走到那一步**。最坑的一点是默认值：`CTX=4096` 和 `NSESSION=4` **各自看着都合理**，
**组合起来才致命**（板上默认启动的就是这个组合，而板上那份模型是 32768 的）。

#### 9.17.4 三道防线

| 层 | 做什么 | 在哪 |
|---|---|---|
| **① 启动闸**（真正的防线） | 从 rope 外置张量的大小反推上下文 N，算出每路 KV 与可容纳路数；`NSESSION` 超了**直接拒绝启动**（`FORCE_SESSIONS=1` 可硬上） | `start_gateway.sh` 的「容量闸」段 |
| **② 对外报数** | `/health` 新增 `ctx_size`（从 `--ctx-size` 解析，取不到就是 `null`） | `rkllm_gateway.py` |
| **③ 页面服从** | 窗口数 = `min(4, sessions)`，增删窗口带守卫、**永远不超过这个数** | `demo_4chat.html` |

判据不解析 JSON，只用大小：**每个位置 256 字节 + 432 字节头** ⇒ 8192 → 2097584 B、
32768 → 8389040 B（`rope_cos_cache` 形状 `[1,4,1,N,16]`）。读不到就**不拦**
（可能是别的命名/别的模型），只在能算出来的时候说话——**读不到就不能当成"安全"**，
这条边界写在脚本注释里。

#### 9.17.5 页面：窗口数由**后端真的起了几个会话**决定

用户原话是"根据上下文的长度**选择**窗口 session 的个数"。实现上**没有**让页面自己按上下文
算一个数，而是让它问 `/health`：

```
GET /health → {"sessions": 2, "ctx_size": 32768, ...}
   ⇒ MAX_SESSIONS = 2、CTX_SIZE = 32768
   ⇒ 建窗数 = min(4, 2) = 2，页面顶部常驻一条容量条：
      上下文 32,768 · 后端会话 2 · 窗口 2/2 · 2 路并发
```

**为什么不让用户直接挑 N**：板上这份导出**物理上就没有 4 路**，能挑一个后端没起的数等于
给一个空头承诺——点下去不是"少用一个会话"，是**把四张卡焊死要重启板卡**（§9.17.3）。
所以"选几个"这条路是**删**出来的：要少就删窗口，要多就被闸挡住并说明原因。

- **＋/×**：`addPanel()` 有守卫（`panels.length >= MAX_SESSIONS` 返回 `null`），
  按钮自己在到顶时 `disabled`，点不动时容量条会追加一句为什么。
- **身份单调不复用**：新窗口拿 `web-<n>`，`n` 只增不减。**故意不复用刚删掉的那个号**——
  复用等于把"新对话"伪装成"老对话的续聊"，而 KV 前缀判据按身份粘会话，这类错误
  **只错不慢**、屏幕上完全看不出来。
- **删窗口立刻交还**：`removePanel` 调 `closeConv(p.convId)`，不等 `IDLE_TTL`。
  面板上的 `面板 N` 只是**显示编号**，删中间一个会重排，与身份无关。
- 顺手修掉一个**按钮存在于 DOM 却没有监听器**的真 bug：`#addwin` 一直在页面上，点了没反应。

**`addPanel` 自己的守卫才是真正的防线**，按钮的 `disabled` 只是提示：`/health` 是异步拿到的
（`capReady`），在它回来之前窗口数是按默认值建的，**只有函数内的判据在两种时序下都成立**。

#### 9.17.6 验证：五个变异体 + 三种配置 + 真板卡

`page_check.js` 新增两段（"建窗与容量条"、"增删窗口"；夹具从 3 组扩到 5 组，多两组是
**2 路**的形态）。**光"跑绿"不能证明"测到了"**——每次改检查都要拿变异体试一遍：

| 变异体 | 期望 | 结果 |
|---|---|---|
| `MAX_SESSIONS` 写死 4 | 容量条与建窗数对不上 | ✅ FAIL |
| 身份复用（`"web-" + (panels.length+1)`） | 新窗口不该复用刚删的号 | ✅ FAIL |
| `removePanel` 去掉 `closeConv` | 删掉的窗口仍占着 `/v1/pool` | ✅ FAIL |
| `addPanel` 去掉容量守卫 | 到顶后还能建出第 N+1 个 | ✅ FAIL |
| 到顶时按钮不 `disabled` | 按钮状态与 `panels.length >= cap` 不符 | ✅ FAIL |

**三种配置本机桩后端全绿**（sessions=4、sessions=6 @8192、**sessions=2 @32768**）；
**真板卡 `CTX=32768 NSESSION=2` 全绿**：启动闸打 `[容量] 这份导出的上下文 = 32768，每路 KV
≈ 406 MB/卡 ⇒ 最多 2 路会话`，后端 `max_ctx_len=32768` + `2 conversations ready`，
`page_check.js` 端到端跑通（两路各 8.72 / 8.79 tok/s、64 tok、7.3 s）。

⚠️ **板上那套回归套件在 `NSESSION=2` 下会报 3 条 FAIL，那不是回归。** 证据两条：
① `git diff` 证明本轮网关改动**只有 print 与 `/health` 加字段**；
② 单独在**空闲池**上跑 `toolcall_check.py` **PASS**（热=32 / 冷=630），
   而套件里那条是 热=630 / 冷=630——套件按 4 路写、会占满池子，2 路下前缀判据必然落空。
另两条同理：`http_scaling.py 1,2,4` 在只有 2 个会话时**显然**跑不了。

**未验证**：① **32K 下"4 个窗口"的形态在板上见不到**（物理上只有 2 路），页面排得下 4 个
窗口这件事只在桩后端（sessions=6）上验过；② 新容量条与 ＋/× 的**视觉排版**只在无头浏览器
里看过，没用真浏览器点过触摸屏/窄窗口。

---

### 9.18 资源条：把"板卡还剩多少、谁在忙"搬到屏幕上（2026-09-20）

用户的一句话：

> 请你将资源使用情况也展示在网页端，资源包括 RK1828、RK3588 的内存，NPU、CPU、温度等，
> 展示是动态的，展示效果请你自己优化

这一节记的是**数据从哪来**和**哪些数只能到这一步为止**，不是页面的 CSS。

#### 9.18.1 三类数据、三条完全不同的来路

| 要看的 | 来路 | 为什么只能这样 |
|---|---|---|
| RK3588 内存 / CPU / 负载 / 运行时长 | 网关自己读 `/proc/stat`、`/proc/meminfo`、`/proc/loadavg`、`/proc/uptime` | procfs 人人可读，与后端是谁无关 |
| RK3588 温度 | `/sys/class/thermal/thermal_zone*`，取最热的 3 个 | 板上 7 个热区，**全部属于 RK3588**（含它自己的 `npu-thermal`） |
| **RK1828 每卡内存** | `RKNN3_QUERY_DEVICE_MEM_INFO`（`rknn3_dev_mem_info`），由**后端**采；取**各 node 之和**（⚠️ 不是 `sys_*`，见 9.18.6） | **设备同一时刻只认一个持有者**：模型跑着的时候 `rknn-smi` 直接 `Failed to initialize rknnsmi` |
| **RK1828 每卡忙时占比** | 后端累计每卡 `session_run` 的耗时 ÷ 墙钟 | **SDK 里没有 NPU 利用率查询接口**，只能自己算 |
| 卡温 / 卡功耗 | —— | 卡是 PCIe 设备（驱动 `rkep`），**没有 hwmon、没有 thermal_zone** |

第 3 行决定了整件事的形状：**这些数只有持有设备的那个进程拿得到**，所以数据必须先由后端
报出来，网关和页面才有得显示。这就把 §9.13 那个"改后端协议"的代价再付一次。

#### 9.18.2 `STAT` 帧：就地答复，而且不碰设备

请求是 stdin 上的一行 `STAT`（无载荷、无 rid），答复是 `STAT <len>` + `<len 字节 JSON>`。

- **为什么载荷用 JSON**：这一帧是**嵌套**的（每张卡一组数）。排空格就得先定死字段序，
  以后加一个字段两边一起改；而两边本来就都在处理 JSON（C++ 侧 nlohmann，网关侧
  `json`），用 JSON 之后加字段是纯加法。
- **为什么不进调度队列**：`REQ` 要排队是因为它要占会话；`STAT` 不占会话。让它排队的话，
  它会被前面那条正在跑的长 decode 挡在后面——而"板卡正忙、想看看它在忙什么"**恰恰是
  最需要它的时刻**。所以它在输入线程里**就地**答复：只读原子量，零设备访问、零锁等待。
- **为什么它答得起**：所有数都是别人早就写好的原子量。内存快照是**推理之后**在卡锁内
  顺手采的（`record_stage_run_stat`），节流 2s——`rknn3_query` 与 `rknn3_session_run` 走
  同一条到设备的传输通道，SDK 没承诺并发安全，所以查询必须在这个卡的 `run_mutex` 里做。

⚠️ **加一种帧 = 后端与网关必须成对换**，这是同一个机制第二次咬人（第一次是 ERR 帧，
§9.12）：网关的读线程对**未知 tag 不消费载荷**，整条流会错位。旧网关读到 `STAT` 会把
JSON 体当下一帧的头行读 ⇒ 之后每个请求都 503。

#### 9.18.3 口径：这几条不写下来，页面上的数就会被读成别的意思

- **忙时用单调钟，不用 `gettimeofday`**：利用率是两次采样的**差值之比**，中间被 NTP 校
  一次表，epoch 时钟会算出一个荒谬的比值（负数、或几百个百分点）。计时点与
  `record_stage_performance` 完全同一个区间，只是钟不同——那个函数保留老口径不动，
  改了历史吞吐数字就不可比了。
- **`mem_at_us == 0` = "一次都没采到"**，与"采过但很旧"是**两回事**：拿 0 当采样时刻去算
  年龄，页面会显示成"53 年没更新"。查询失败时**保留旧值且不更新时刻**——宁可显示"这个数
  12s 没更新过"，也不要拿旧值冒充新的。
- **缺值画 `—`，绝不画 `0%`**：`0%` 是在说"这张卡闲着"，`—` 才是在说"不知道"。第一次采样
  算不出 CPU%/忙时占比（没有前一个点），这时也是 `—`。
- **"还没采到"与"过期"分开**：前者是页面刚打开，后者是 `stats_age_s > 6`，只有后者标黄。
- 页面上常驻两句说明："**NPU 是卡级忙时占比**（SDK 没有利用率查询接口）"、"**温度只有
  RK3588 的热区**"。不写这两句，看的人会以为那是卡温、以为 NPU 那个数是 SDK 报的。

#### 9.18.4 一个调用点漏了，第 0 张卡就是一张"空卡"

`session_run` 有**两个**调用点：`run_stage_worker`（各段）与 `run_pipeline_once` 里**就地跑
的 stage0**（stage0 不走 worker）。第一版只在 worker 里记了观测，于是第 0 张卡永远报
`run_calls:0` / "还没采到内存"——页面上三张卡在动、一张空着，**看着像那张卡坏了**。

抓到它的是主机侧的 t12（§9.11 那套夹具）。有意思的是抓到它的原因：桩不喂 batch，所以
2 段夹具里 stage1 的 worker **根本不会执行**，于是"漏掉的那个调用点"成了唯一会跑的那一个
——测试第一次跑就红在"跑过之后 run_calls 动了（实际 `[0,0]`）"。**靠读代码发现不了这个**：
两处代码写得都对，错的是"只有一处被写"。

修法不是补一行，而是把三行收成一个 `record_stage_run_stat(stage, busy_us)`，两处都调它。

#### 9.18.5 验证：本机全绿，然后上板

- 网关自检新增 `selftest_system`：一份**假 `/proc` + 假 `/sys` 夹具**（`tempfile.mkdtemp` +
  覆盖 `root`），覆盖 CPU 差值算 25.0%、满核 100%、零差值 0%、**只有 iowait 变化时 0%**、
  负差值钳到 0、`MemAvailable` 而不是 `MemFree`（62.5% vs 87.5%）、温度里混垃圾项与缺
  `type` 的项、坏载荷被丢弃且 `stats_seq` 不失步。
- 变异体：网关侧 9 个（含"把 iowait 当忙""用 10^9 当 GB"）**全被抓**；页面侧 12 个
  （含"缺值画 0%""旧数据不标过期""热区不排序"）**全被抓**；主机 C++ 侧 t12 的 3 个
  （STAT 头行多一个字段 / 不记观测 / 拿阶段数凑节点数）**全被抓**。
- `page_check.js` 对本地桩后端 `sessions=2` 端到端 `===== 全部通过 =====`。

**本机全绿并不等于对**——真机上立刻抓到两个错（9.18.6）。修完之后：主机侧工作区版全绿、
`HEAD` 版与 **10 个**变异体全在该失败处失败；网关自检 **124 PASS / 0 FAIL**；真板卡
（2 路 @32768）上四张卡的数字与 `rknn-smi` 那组旧记录**逐项吻合**；`page_check.js` 对
**真板卡**（2 路窗口）也端到端全过——不只是对着本地桩。

**仍未验证**：① 卡温拿不到这件事只在 `rknn-smi`/驱动层面确认过，没在板上再抓一次；
② 资源条的**视觉排版**只在无头浏览器里看过（真机上看的是文本，不是像素）。这一条在
§9.19 换排版之后更吃紧：桩 DOM 没有 CSS，"一块一张卡、看起来够不够松"只能靠眼睛。

#### 9.18.6 上板实测：两个"本机永远看不出来"的错

**错一：整卡内存取错了字段。** 页面上每张卡写着「内存 19.0 MB」，而同一时刻 `rknn-smi`
报 93% 占用。`rknn3_dev_mem_info` 里有两组字段：

| 字段 | 是什么 | 板上实测 |
|---|---|---|
| `sys_total` / `sys_free` | **本机侧的小块内存**（驱动/传输缓冲），与模型放不放得下无关 | 19.0 MB / 18.7 MB |
| `node_mem_info[i].total/free` | 每个 node 的内存，**模型真正落地的地方**；整卡 = 各 node 之和 | 8 × ~634 MB ≈ **5.06 GB/卡** |

所以这一格必须是 `Σ node.total` / `Σ node.free`，另加"最紧那个 node 的余量"（最先撞墙的一格）。
改完之后的数：0~2 号卡 `mem_used_pct = 92.4%`、3 号卡 `95.4%`、`node_min_free` = 33.4 MB ×3 +
**13.25 MB**——正好落在 §9.17 记的「2 路 @32K ⇒ smi 93/95%」上，**这就是"取对了"的判据**：
页面的数能和另一条独立来路的数对上。

> 这个字段的含义在 `--probe` 那段老注释里就写着（"真正要看的是每个 node 的可用量"），
> 取的时候没照做。教训不是"注释没用"，而是**同一结构体里两组数长得都像"内存"**时，
> 得拿一条外部判据（`rknn-smi`）对一次。

**测试为什么没拦住**：主机桩对设备内存一律回零 ⇒ `Σ node` 和 `sys_*` **都是 0**，
取哪个字段在主机上完全等效。修法是让桩**填一张确定的假表**，而且故意让两组数差得远：
`sys_*` 就用板上量到的那两个 19 MB 的数，node 数取 **3**（≠ 夹具的 2 段，顺带让"拿阶段数
凑节点数"那个变异体继续有效）。新增变异体 `stat_use_sys`（改回取 `sys_*`）现在会被 t12 抓住。

**错二：`busy_pct` 每隔几次从 `0.0` 变成 `None`，页面在数字和 `—` 之间闪。**
利用率是"两次 STAT **答复**之差 ÷ 墙钟"，而两次之间隔着的是"**谁能拿到新 doc**"：

- `STAT_WAIT` 当时钉 0.35s，板上实测**偶发**答复会晚于它 ⇒ 超时的调用拿到的是**上一份**
  doc（`now_us` 没动 ⇒ Δt = 0 ⇒ 算不出比值 ⇒ 给 `None`）。同一台机器连问 10 次，
  正常时 HTTP 全程 7~11ms，出问题那几次则卡在等待窗口上。
- 更隐蔽的是第二半：那次采样**无条件推进了窗口**。于是即使下一次拿到了新 doc，Δ 也只剩
  后半段——观测被悄悄切掉一截，而屏幕上什么都看不出来。

**两处一起修**：① `STAT_WAIT` 放到 1.0s（这条路径只服务 `/v1/system`，不在聊天请求的路上）；
② 同一份 doc 连着采两次时**既不推进窗口、也把上一次算出的百分比原样给回去**——数据本身
没变旧，`—` 要留给"真的没有值"。第 ② 条顺带修掉了另一个场景：**多人同时开着页面**时，
晚到的那个请求以前会拿到 `—`，现在拿到的是同一个数（窗口是"上一次真拿到新 doc 到现在"
这一段，谁先问谁算）。

> 两条都属于同一类：**显示层的"不知道"会被误读成"没有"**。所以这里的选择一律是
> "宁可用上一份真数据 + 标明它多旧"，而不是吐一个 `None` 或一个被切短的窗口算出来的比值。

**修完之后的真机确认**（同一个 2 路 @32768 后端，只把网关换成新的一版）：

| 量什么 | 修前 | 修后 |
|---|---|---|
| 连采 8 次（间隔 1.5s）`busy_pct` | 数字与 `None` 交替出现 | **8 次全是 `0.0`，一次 `None` 都没有** |
| 同一组的 `stats_age_s` | 偶尔一次采样跨过好几个间隔 | 0.0~0.7s ⇒ 每次都拿到了新 doc |
| 聊两句时的 `npu.busy_pct` / 每卡 | — | 20.6 / 18.2~21.5（四张卡一起上） |
| 停手 3 秒后 | — | 回落到 **`0.0`**（真的闲着，不是 `—`） |
| 两轮之后每卡 `run_calls` | — | **四张卡都是 40**（含最前面那张 stage0） |

最后一行是**顺手捞到的另一条确认**：§9.18.4 那处「`session_run` 有两个调用点」的修复，
主机 t12 只证明了"代码路径都在记"，真机上这 40/40/40/40 才说明**四张卡一条没漏**。
`node_min_free` 那一列仍与 `rknn-smi` 的占用率对得上（3 号卡 13.25 MB ↔ smi 95.4%）。

---

### 9.19 资源那一块：挪到页脚，并把开销压到"与看的人数无关"（2026-09-20）

用户两点要求：① 设备资源使用情况**放到网页底部**、布局别太紧凑；② **尽量减少这个展示功能
对硬件资源的占用**。第 ① 条是位置与排版，第 ② 条要先把"这一块到底花掉什么"数清楚。

#### 9.19.1 一次快照花掉的东西（以及为什么它值一提）

一次 `/v1/system` = 网关读服务器本机的 `proc/stat`、`proc/meminfo`、`proc/loadavg`、
`proc/uptime` + 7 个热区的 `type`/`temp`（≈ 18 次文件打开）+ **一次 STAT 帧往返**
（写一行、读一行 JSON）+ 会话池快照。

这里的每一分钱都是**被观测的那台机器**出的：读的是它自己的 `/proc`，STAT 帧的答复线程
也在它上面跑，而**四路推理就跑在同一颗 RK3588 上**。所以"多开几个页面看状态"这件事本身
会让被看的机器变慢——这是资源展示功能特有的一种反噬：**看的人越多，看到的数越差**。
§9.18 里的 `STAT_MIN_INTERVAL = 0.9` 只是节流了"向后端要帧"这一步，`/proc` 那一半
是每个请求都要重新读的。

#### 9.19.2 页侧：两个档位 + 同一份 doc 不重画

- **节奏**：生成中 **2s**、待机 **6s**（原来是一律 2s）。待机时真正会变的只有温度和内存，
  用不着 2 秒问一次。切到后台停轮询是原有的（§9.18.5）；新增的是**新一轮刚开始生成时
  立刻补一拍**——不然待机档最长要等 6 秒才看到卡忙起来，而"点了②之后卡是不是真的忙了"
  恰恰是演示时最想看的那一帧。
- **重画**：拿到的 doc `ts` 与上一次**画到屏幕上**的那份相同 ⇒ **连 DOM 都不动**。
  这一条只有在网关侧有了 1 秒快照缓存之后才真的省得下来（同一秒里的第二拍必然拿到同一份，
  见下），省掉的是整块 `innerHTML` 重写引发的重新排版——数据一个没变，排版白花一次。
- ⚠️ **判断里必须带 `ts != null`**：应答里没有 `ts`（老网关、或者中间有人自己写了个转发）
  时 `undefined === undefined` 成立 ⇒ 这一页**从此不再刷新**。屏幕上就是数字冻在打开那一帧，
  看着还挺正常——这类"看起来正常的冻结"只有把判据写死才防得住，所以 `page_check.js` 里
  专门有一条拿**没有 ts 的应答**去打它。

#### 9.19.3 网关侧：一秒只采一次，热区表只问一次

**`SYS_CACHE_S = 1.0` 的快照缓存**：1 秒内不管几个请求来问，**只采一次**，后来的拿到同一份
（采的时候持锁，后来的人在锁外等现成的）。这一条把开销从"与看的人数成正比"变成
"每分钟最多 60 次，跟几个人看没关系"。它同时是 9.19.2 那道闸的前提：**没有缓存，
同一秒里的两次轮询必然是两个不同的 `ts`，页面那一半的节省等于不存在**。
两个实现上的点：

- 时间戳取**采完**的时刻，不是开始采的时刻——采这一份本身可能要等 STAT 一秒钟，
  按开始时刻算的话它一出锁就是过期的，缓存等于不存在。
- `max_age=0` 可以绕开缓存强制重采（自检与排障用）。

**热区表只问一次**：「有哪些热区、各自叫什么」开机就定下来了（板卡上 7 个区 ⇒ 每拍
1 次 `listdir` + 7 次读 `type`），现在只缓存这张表，每拍只剩读温度那 7 次。**只缓存列成功
的结果**——本机（没有 `/sys`）那种读不到的情况不缓存，免得偶然一次失败被记成"这台机器
没有热区"。

#### 9.19.4 验证：两个"全绿也说明不了问题"的陷阱

网关自检 **130 PASS / 0 FAIL**（新增 6 条）。`page_check.js` 对桩网关
`sessions=2 @32768` 与 `sessions=4 @8192` 两种配置全过（新增 7 条：位置在 `#grid` 之后、
两个档位、`ts` 相同不重画 / `ts` 变了必须重画 / 没有 `ts` 不许冻住、`sysWake` 真的补出
那一拍）。本地对 8099 桩网关三次连问拿到**同一个 `ts`**、隔 1.3s 再问换了新的。

两个陷阱都值得记下来：

| 陷阱 | 只写"正常那一半"会怎样 | 所以补的那一条 |
|---|---|---|
| 缓存 | 只测"1 秒内不重采"⇒ 一个**永不过期**的写法（忘了比时间、或时间戳写成 0）照样全绿，而它在板上是资源区**冻住不动** | 睡过 `SYS_CACHE_S` 之后再问，必须**重采**、`ts` 必须是新的 |
| 跳过重画 | 只测"同一份不重画"⇒ 一个**永远跳过**的写法全绿，屏幕上同样是冻住 | 换个 `ts` 必须重画；没有 `ts` 时不许当成"和上一份一样" |

两条陷阱是同一个形状：**这个功能坏了的样子是"不动"，而"不动"也正是它正常工作时的样子**
（待机时温度内存本来就不怎么变）。所以判据只能是"给它一个必须变的条件，看它变没变"，
不能是"看它有没有在动"。

**变异体（9 个，全被抓）**：页侧 6 个——`send()` 不调 `sysWake`、待机档写死 2s、
忘掉 `ts` 判空、永远不重画、四张卡那一行整块不画、位置判据反过来；网关侧 3 个——
缓存永不过期、根本不缓存、采完不盖时间戳。第 1 个尤其值一提：前面几节只证明了
`sysWake` 这个函数有用，**证不了它被接在生成那条路上**，所以最后那一条断言走的是
真的 `send()`（塞一句输入、发一轮）。

**顺带修掉检查脚本自己的一处尾巴**：为了验上一条，`page_check.js` 最后一节真的发了一轮，
而它收尾直接 `process.exit` —— 那段对话的租约就留在池子里了。板上占着 `IDLE_TTL`（默认
300s，自己会好），本地桩网关（`--idle-ttl 0` = 不回收）则是**永久**占着，同一个桩网关连跑
...
### 9.20 换端口 + 收紧 CORS，以及"整页一屏"（2026-09-20）

用户的第 2 条要求："对话框下面有很大的空余内容……资源展示放在网页底部约占整个网页的
1/4 到 1/5 即可，将空白填满，可以适当将对话框拉长"；第 1 条与它无关，是上一节那个
"8080 暴露出去有没有隐患"的收尾——用户当场选了**换端口 + 收紧 CORS**（明确的**不是**：
共享令牌鉴权、HTTP 层连接数/请求体上限，两条这一轮都没做）。

#### 9.20.1 先说清楚：换端口本身不是安全控制

`start_gateway.sh` 的 `HOST` 默认就是 `0.0.0.0`（§9.7 定的，为的是工作流 Agent 能从别的
机器连进来），也就是说**同一个局域网里的任何人本来就连得上**，扫一遍端口就知道服务在
哪。8080 唯一特殊的地方是它排在几乎每个扫描器的**第一页名单**上，而这套服务**没有鉴权**
（R12）——被扫到就等于被用上。所以：

- 挪端口把这块板卡从"默认名单"上拿开，**降低被随手扫到的概率**，但挡不住任何有意找它的人；
- 真正被修掉的那个洞是 **CORS**（下一节），它不是"谁连得上"，而是"**谁能替别人连**"。

端口改成 **18280**（`--port` 默认值、`start_gateway.sh` 的 `PORT`、所有 demo/验收脚本的
BASE 一起改）。`PORT=8080 ./start_gateway.sh` 仍然可用。⚠️ **改端口不是单向的**：
同事那条 SSH 隧道、cc-switch 的 Base URL、浏览器书签都得跟着改。

#### 9.20.2 真正该修的洞：`Access-Control-Allow-Origin: *` + 不要凭据的 API

原实现无条件回 `Access-Control-Allow-Origin: *`。这个服务的 API **不带凭据**（没有 cookie、
没有 token），所以 CORS 在这里不是"防别人偷数据"，而是：**局域网里任何人打开的任意一个
网页**，只要页面上有一段 JS 指向这块板卡，浏览器就批准它去读 `/v1/system`、去发起生成、
去用 `/v1/pool` 返回的 key 把**别人的**对话 `close` 掉。受害者不需要做任何事，只需要
访问一个被人安排好的页面。

而演示页是**同源**的（`GET /demo`，同一个 host:port 取回来的），那个 `*` 对正经用法
**一点用都没有**。所以：

| 项 | 改前 | 改后 |
|---|---|---|
| 默认跨源 | 一律放行（`*`） | **一个 CORS 头都不发** |
| 放行方式 | 无 | `--cors-origin` / `CORS_ORIGIN` 显式给**一个**来源 |
| 匹配 | 不适用 | **精确匹配**（不做子串、不做通配）；写 `*` 才真的是 `*` |
| 预检（OPTIONS） | 回一套 `Allow-*` | 没配置来源时**只回空 204** |

放行时同时回 `Access-Control-Expose-Headers: X-KV-Reuse`——那个头是给**显式放行**的
前端看的，同源页本来就读得到。

四个地方发响应（`_json` / OPTIONS 预检 / `GET /demo` / SSE 流），**各写各的**，所以判断
逻辑收成一个纯函数 `cors_headers(allowed, origin)`：默认参数下**永远返回空**，这条性质是
整条防线里唯一重要的东西——自检专门拿它变红来钉住（见 9.20.3）。

**改端口不改帧协议**，后端二进制不用重编；但**换网关必须成对换**这条老规矩仍然成立
（STAT 帧那次已经吃过一次）。

#### 9.20.3 验证：函数返回值对 ≠ 线路上发出去的头对

新增 `selftest_cors`（19 条），分两层：

- **纯函数**：默认 `{}`、给了来源但没带 `Origin` → `{}`、精确匹配回显来源 + Expose、
  不匹配 → `{}`、**前缀不算匹配**（`a.example.evil.com` 打 `a.example` 必须落空——
  子串匹配是这类白名单最经典的写法错误）、显式 `*` 才回 `*`、`Gateway(None, None)`
  的 `cors_origin` 是 `None`（默认值没被谁偷偷改掉）。
- **线路上**：真起一个 `ThreadingHTTPServer(("127.0.0.1", 0), Handler)`，用 `http.client`
  打真的请求、读真的响应头——断言 `/health` 与 `/demo` **一个 ACAO 都不带**、预检是 204
  且不带 `Allow-Methods`/`Allow-Headers`；再把 `cors_origin` 设成 `http://a.example`，
  断言匹配的来源被回显、子串来源不被回显、预检里出现 `X-Conversation-Id`；最后
  `/demo` 不带 `Origin` 照样能用（同源页不能被自己这道门挡住）。

为什么非要真发请求：这四处是**四段独立的代码**，纯函数对了只说明"算得对"，说明不了
"每处都用了它、而且回的是同一个答案"。

**变异体**：把 `cors_headers` 改回"永远发 `*`"（要注入在守卫**之前**，只在守卫上做手脚
的改法不构成有效变异体）⇒ **16 条 FAIL**。

#### 9.20.4 整页一屏：让对话区吃掉剩余高度

**症状**：面板写死 `height: calc(50vh - 60px)`、资源块按内容长，三块加起来够不到一屏，
屏幕上就是"对话框下面一大片空白"。

**改法**（`demo_4chat.html`）：整页变成一列 flex。`html,body{height:100%}` +
`body{display:flex;flex-direction:column;overflow:hidden}`，标题、容量条、汇总条
`flex:none`，**对话区 `flex:1 1 auto; min-height:0; overflow:auto` 吃掉剩下的全部高度**，
资源块 `flex:0 1 auto` 按内容自然长。面板不再有写死高度，两列用
`grid-auto-rows:minmax(0,1fr)` 平分。

用 `height:100%` 而不是 `100vh`：移动端地址栏收起/展开时 `100vh` 是不变的"大视口"高度，
页面会稳定地缺一条。

**同时把资源块自己压矮**（用户要"底部约占 1/5~1/4"）。原来上下两块（上面
`RK3588 / 热区 / 服务`，下面四张卡排一行）要 **376px**，1080p 上占 37%。现在改成左右两栏
（`.swrap` = `minmax(600px,1fr) minmax(0,1fr)`，左列三块、右列四张卡，
`grid-template-columns:repeat(auto-fit,minmax(…,1fr))`）⇒ 高度从"两者相加"变成
`max(左, 右)`，**实测 249px**：1080p 上 **25%**、1440p 上 **17%**。**一块内容都没少、
也没挤紧**，只是不再白占一整行。窄屏（`max-width:1050px`）退回上下两块。

⚠️ 各块的**最小宽度不能再往下压**：块头那行是 `RK1828 #0 ctx 8192`，低于实测值卡片就换行，
一整行的高度又长回去了。

#### 9.20.5 这个改动只能靠读源码来验

`page_check.js` 跑在桩 DOM 上，**没有排版引擎**——空白多不多它看不见，能看见的只有
`<style>` 里的规则。所以第 ⑩ 节是一个**源码级的 CSS 检查器**：先剥掉注释、再按花括号
配对扫出 `{选择器, 声明体, 所属 @media}`，`decl(sel, prop, media)` 取某条属性的**最后**
一次声明（CSS 的就近覆盖）。

判的东西就是"会让空白回来的那几条"：

- `html`/`body` 高 `100%`；`body` 是 `flex-direction:column`；
- `main` 的 `flex-grow ≥ 1` **且**有 `min-height:0`（flex 子项默认可伸缩下限是内容高，
  少了这一条整页还是会被撑出屏幕，滚出第二屏）；
- **任何选择器都不许用 `vh` 写死高度**——这一条正是原来那个坑（报错信息里点名是哪个选择器）；
- 资源块封顶在 1/5~1/3 屏；`.syssec` 自己 `overflow:auto`（矮屏兜底）；
- `.swrap` / `.sg-host` / `.sg-cards` 这三个类名**在 JS 和 CSS 两边都在**——它们靠字符串
  联系，谁单方面改名页面**照样跑、功能检查全过**，只是悄悄长回 376px 的上下堆叠；
- `.swrap` 是两列、窄屏媒体查询把它塌成一列；标题/两条线是 `flex:none`。

⚠️ **解析前必须先剥注释**：注释里恰好写着 `calc(50vh - 60px)` 这种"以前的写法"，不剥的话
一个**纯注释**就能让检查变红（变异体专门验了这条：只把 vh 写进注释不许红）。

**测量方法**：无头 Chrome（`--headless=new --dump-dom --virtual-time-budget=6000`）临时
注入一段测量脚本、把 `clientHeight`/`scrollHeight`/`body.scrollHeight`/
`documentElement.scrollHeight` 打成一个 `<pre id="MEASURE">`。1920×1200 / 1600×900 /
1440×900 / 1366×768 / 1280×800 / 1000×800 六种尺寸下
**`body.scrollHeight == documentElement.scrollHeight == innerHeight`**（整页恰好一屏、
没有溢出、底部也没有空白），另用 `--screenshot` 看过一眼：对话面板拉长、底部资源块完整
可见、四张卡数得出来、说明行在。

**布局变异体（4 个，全被抓）**：面板写死回 `vh`、去掉资源块封顶、去掉 `swrap` 类名、
去掉窄屏媒体查询。
...
| 项 | 结果 |
|---|---|
| 网关自检 | **149 PASS / 0 FAIL**（新增 19 条 CORS） |
| CORS 变异体 | 还原成"永远发 `*`" ⇒ **16 FAIL** |
| 线路级复核 | `curl -I` 打 `/health`、`/demo`、`OPTIONS` 预检、`_json`、SSE —— **一个 ACAO 都没有** |
| `page_check.js` | 对 `http://127.0.0.1:18280` 桩后端 **===== 全部通过 =====**（新增第 ⑩ 节） |
| 布局变异体 | 4 个全被抓（含"只把 vh 写进注释不许红"） |
| 整页高度 | 六种窗口尺寸下 `body.scrollHeight == innerHeight`，资源块 249px（1080p 25% / 1440p 17%） |
| 脚本语法 | `bash -n` 过（`start_gateway.sh` / `run_all_board_tests.sh`） |
| 板上 | 已重启到 **18280**，`page_check.js` 端到端 **`===== 全部通过 =====`**、`/demo` 与工作区逐字节相同、线路级 CORS 复核无 ACAO（§9.20.7） |

#### 9.20.7 上板：换端口要重启网关，客户端要跟着改

板上那套是 §9.19 的页脚版，本来跑在 `:8080`。换端口必须重启网关（监听地址是启动参数），
流程照旧：往帧管道写 `QUIT\n` 停旧的 → `PORT=18280 CTX=32768 NSESSION=2` 起新的 →
验 `/health`（`sessions:2` / `ctx_size:32768` / `backend_alive:true`）→ 本机 `curl` 复核
**不带 ACAO** → 板上跑一遍 `node page_check.js http://<板卡IP>:18280`。

⚠️ **重启网关 = 模型重新加载 240s**，别为了"顺手也试试别的"反复重启。
⚠️ 停进程不能靠 `kill -INT`（它继承了 `SIG_IGN`），走 `QUIT` 帧或 `SIGTERM`。

**本轮实跑结果**（2026-09-20）：

| 项 | 结果 |
|---|---|
| `/health` | `sessions:2` / `ctx_size:32768` / `backend_alive:true` |
| 线路级 CORS | 带 `Origin: http://evil.example` 打 `/health` 与 `/demo` ⇒ **一个 `Access-Control-*` 都没有**；`OPTIONS` 预检 ⇒ **204 + `Content-Length: 0`**、无 `Allow-*` |
| 旧端口 | `:8080` **拒连**（TCP 000） |
| `page_check.js`（本机跑，指板卡） | **`===== 全部通过 =====`**；两路 8.77 / 8.70 tok/s、各路速率和 17.47、落点会话 1/0 |
| `/demo` 是板上那一份 | md5 **`5be0c976…`** 与工作区**逐字节相同** |
| 无头截图 1600×900 | 整页恰好一屏、对话面板拉长、资源块在页脚占 **≈26%**、四张卡数得出来、说明行在；（那一刻板上正跑着 2 路 ⇒ 卡内存 93/93/93/95%、`node 32/32/32/13 MB`，与 §9.18.6 记的数一致） |

⚠️ **`page_check.js` 要在有 node 的机器上跑、用 `http://<板卡IP>:18280` 指过来**——
板卡本身**没装 node**（`node: command not found`）。

**用户侧要跟着改的**（改完端口不通知等于把同事的服务断掉）：同事那条 SSH 隧道
（`<板卡IP>:8080` → 本地 `127.0.0.1:18082` 里的 `8080` 那半）、cc-switch 的
Base URL（`http://<板卡IP>:8080`）、浏览器书签。

---

### 9.21 底部资源看板：七块卡片等高齐底、「服务」两列、宽度不够时整段折（2026-09-20）

#### 9.21.1 用户要的三件事

用户给的是结构性的排版要求，不是"看着调一下"：① 七块卡片包在一个统一容器里，
`align-items: stretch`，**左侧 RK3588 相关区块的最底边必须与右侧 RK1828 区块的最底边
在同一水平线上**（原文："解决目前视觉上高低不平的问题"）；② 「服务」从单列垂直列表改成
`grid-template-columns: 1fr 1fr`；③ 统一卡片之间的外间距与内边距，内容少的卡片内部贴顶，
"避免产生异常的视觉留白"。

**先量出来再说**：拿板上真的 `/v1/system` 喂进页面，无头 Chrome 量 `getBoundingClientRect()`。
改前 1600×900 下左侧三块的底边在 771、右侧四张卡在 723——**差 48px**；而 `auto-fit` 让
1366 与 1280 更糟（右栏折成 3+1、左栏折成 2+1，整块从 245px 长到 293px）。

#### 9.21.2 等高是三步叠出来的，缺一步都不齐

这是这一轮最值得记的一条：**三处都像是对的，但少任何一处都不齐**。

1. 外层 `.swrap` 的 `align-items` 用**默认的 `stretch`**（原来写的是 `start`）⇒ 两个子网格
   等高，高度 = 左三块与右四块里最高的那一块；
2. 子网格 `.sgrid` 的 `align-content` **也要 `stretch`**（原来是 `start`）——**这一条最容易
   漏**：单行的网格在 `align-content:start` 下只占内容高，外层传来的 stretch 根本落不到卡片上，
   前一条改了也白改；
3. `.sblock` 默认 `align-self:stretch`，每块被拉满这一行；再给它 `display:flex` +
   `flex-direction:column` + `justify-content:flex-start`——**齐平的是底边，不是里面的行**，
   内容少的那块（热区只有一行温度）多出来的空白全留在底部，七块的"块头 + 第一行数据"
   于是横向对齐成一排（用户第 ③ 条要的正是这个）。

列数**写死**（左 `repeat(3,minmax(0,1fr))`、右 `repeat(4,…)`，外层 `3fr:4fr`），换掉
`auto-fit`。理由是量出来的：`auto-fit` 会在 1500px 以下自己折行，一折行两组的高度就再也
对不上；写死之后卡片只是变窄，**永远是一条线**。3:4 这个比例不是凑的——它正好让七张卡
**一样宽**（1920 上实测七块全是 255px）。

#### 9.21.3 「服务」两列，但窄屏必须退回一列，而且两头都要有界

`.svc` 改成 `grid-template-columns:repeat(2,minmax(0,1fr))`，5 行变 3 行，这一块就不再是
七块里最高的那块。但**不能一路两列到底**：

- 1500px 以下卡片掉到 200px 上下，每格只有 ~65px，"数据 0.0s 前"装不进一格就自己折成两行，
  五个格子共折出六行——**比一列五行还高**（1280 上实测 210px vs 192px）；
- 而 1180px 以下这份页面又退回上下两组，卡片变回 360px 宽、两列完全放得下，这时单列反而
  白把这一块顶高 48px（实测 217px vs 235px）。

所以那条媒体查询**两头都有界**：`@media (max-width:1500px) and (min-width:1181px)`。
只写下界忘上说、只写上说忘下界，都是"页面照样跑、只是白高一块"。

#### 9.21.4 宽度不够时的两种"折"：整段折，不许断词，更不许溢出

**温度那一行溢出过**。它是七块里唯一一条长度不设上界的（三个热区名 + 各自度数，
"温度 soc 31° · bigcore0 31° · bigcore1 31° +4"在 1920 上 231px，正好塞进 255px 的卡），
而 `.m` 是 `white-space:nowrap` ⇒ **卡片一窄到 207px 文字就顶出边框、压在隔壁那张卡上**
（1600 / 1366 / 1280 三档实测都报 `scrollWidth > clientWidth`）。改法不是缩字号，是让它折：
整行改成会折的 `inline-flex`，每个热区各自是一个不折的格（`.ti`，名字和度数永远在一起），
并且要 `flex:1 1 auto; min-width:0` 才拿得到行宽——**flex 项默认 `min-width:auto`，会硬撑到
内容宽，那就永远不折**。

⚠️ 分隔符 `·` 是**真文本**，写在每一格的开头。第一版挂在 CSS 的 `::after` 上，被检查判红了：
**生成的内容复制不出来、读屏软件时常也不念**——屏幕上明明是"npu 81° · bigcore0 51°"，
复制下来成了"npu 81°bigcore0 51°"。同一个文件里别处为"标签与值之间那个空格"写过同样的
理由。格间那个空格是**不折空格**（普通空格会落在折行点上被吃掉，点就贴上数字了）。

**"内存"那一行断成过四截**。`.srow` 是 flex 且 `align-items:center`，尾注（"6.2 GB /
15.6 GB"）放不下时会在**自己内部**一个词一个词地折——实测 1280 上那一行 74px 高、
屏幕上一列断词。改法是给 `.srow` 加 `flex-wrap:wrap`，再把尾注标 `.m` 成为整体，
让它**整段挪到下一行**去。

#### 9.21.5 顺带修掉一处静默截断：资源块不该被 flex 压小

`.syssec` 原来是 `flex:0 1 auto`。它和 `main` 的 `flex-shrink` 都是 1，矮屏上多出来的高度
**按基准尺寸在两块之间分摊**（不是"先挤 main"），于是它被压到**比 `max-height` 还小**——
实测 1366×768 下 249px 的内容只分到 185px，底部那句"NPU 是卡级忙时占比（SDK 没有利用率
查询接口）"被切掉了。**而那句正是防止屏幕上两个百分数被当成硬件读数读走的**：切掉它不会
报错、数字也全对，只是有人会把 55% 当成 NPU 的真实利用率读走。

改成 `flex:0 0 auto` 之后它的高度只有两种可能：装得下就按内容（1600 上 221px）、装不下就
正好卡在 `max-height`（1366 上 227.8px），**要挤全部由 `main` 让**（`main` 本来就是
`min-height:0` + 自己滚的那一个）。

#### 9.21.6 验证：15 个布局变异体，逐个试过

这一节的检查仍然只能查源码（桩 DOM 没有排版引擎），但**几何**这一次是真的量过的——
无头 Chrome 注入测量脚本、喂板上真 `/v1/system`，报每块卡片的 `top/bottom/width/height`、
每条 `.srow` 的行高、`scrollWidth > clientWidth` 的溢出标记：

| 窗口 | 七块卡片 | 底边 | 资源块占屏 |
|---|---|---|---|
| 1920×1080 | 255×117 · 一行 | 全部 929 | **21%**（`scrollHeight == clientHeight`，不滚） |
| 1600×900 | 210×138 · 一行 | 全部 762 | 28% |
| 1366×768 | 176×165 · 一行 | 全部 640 | 34%（卡在 `max-height`，块内滚） |
| 1280×800 | 164×165 · 一行 | 全部 661 | 34%（同上） |
| 1180×800 | 上下两组 | 各自一行内齐 | 34% |

同一把尺子改前：1600 上底边差 48px、1366 上资源块占 **49%**、1280 上 **54%**。

**15 个变异体全被抓**：`align-items` 改回 `start` · `align-content` 改回 `start` ·
`.sblock` 加 `align-self:flex-start` · `justify-content` 改 `center` · 列数改回 `auto-fit` ·
左栏改 `repeat(2,…)` · `flex-shrink` 改回 1 · `.svc` 改单列 · 媒体查询去掉下界 ·
JS 里 `.svc` 改名 · 温度行去掉 `flex-wrap` · 分隔符从文本里删掉 · 分隔符搬回 CSS ·
`.srow` 去掉 `flex-wrap` · 尾注去掉 `.m`。

⚠️ 其中**"分隔符必须是文本"这一条只有读页面真跑出来的文本才查得到**——桩 DOM 里 `::after`
根本不生成文本，所以它和"热区按温度排前 3"那条断言放在同一节（夹具那一节读的是
`renderSys` 真跑出来的 HTML）。另外**尾注那条断言必须指到具体那一段**（带字节数、带
`title`）而不是泛泛地找 `class="m dim2"`：页面上有三处尾注，泛泛地找的话改坏其中一处、
另外两处照样命中，那条断言就成了空转的——**第一版就是这样，变异体把它抓出来了**。

#### 9.21.7 上板

换演示页**不用重启网关**（`_static_demo` 每次请求现读磁盘），所以这一轮只传了一个文件、
没碰模型。`/demo` 取回来与工作区**逐字节相同**；`page_check.js` 对**真板卡**（2 路窗口 /
32K）`===== 全部通过 =====`；1920 与 1366 两张无头截图确认七块等高、底边齐。

⚠️ 板上那台是**在用的**机器：这一次开工时池子里躺着一个上一次遗留的 `id:web-baseline`
租约、占着 session 0 且状态是 `busy`（`idle_s` 为 `null`，**回收器只收 idle 的，所以它永远
不会自己好**），第一遍 `page_check.js` 因此报"2 路没有落在 2 个不同会话上"。`POST
/v1/conversations/close {"key":"id:web-baseline"}` 清掉之后全绿。**对着真板卡跑检查之前
先看一眼 `/v1/pool`**，不然会把自己上次的尾巴当成页面的毛病。
...
| **容量（实测）** | **每卡硬上限 5 个 session**（第 6 个失败）；**4 路 × 3894 token 上下文已验证全部成功** → 方案目标有 1 个 session 余量。⚠️ **这个 5 是 4096/8192 导出上的数，它随上下文长度下降**：**32768 导出每卡只够 2 路**（每路 406.7 MB/卡 > 1078÷3，§9.17.1）。启动闸按模型算一遍并拒绝超配（§9.17.4） |
| **每会话成本** | **64.4–70.7 MB/卡**（4096 导出），在 `session_init` 时按满上下文**一次性预分配**，与**这一路实际用掉多少 token** 无关——但**与上下文长度成正比**（⚠️ 别把这句读成"上下文不影响容量"）：8192 → 101.7、**32768 → 406.7 MB/卡**（§9.17.1，单价 ≈ 12.4 KB/token/卡） |
| **权重** | ✅ **共享，不 per-session 复制**（否则第 2 个 session 就会 OOM） |
| **KV 隔离** | ✅ **隔离已证**：两个不同 prompt 各 12 步 decode 逐步交错，序列逐 token 一致 |
| **模型** | ✅ **不用重新转换**（用 4096 上下文时）。单 KV group。⚠️ **2026-09-20 更正**：这里原先写"每卡 5 session 的上限与上下文大小无关，降 ctx 重导不能提高并发数"——**那句话是错的**（只在 4096/8192 上偶然成立，因为 KV 不是瓶颈）。容量 = **上下文 × 路数**：**32768 导出每卡只够 2 路**，此时"降 ctx 重导"正是唯一的正解（§9.17.2）。**2026-09-17 已换成 8192 上下文导出**（判据是 `rope_cos_cache` 形状 `F16 [1, 4, 1, 8192, 16]`，按 `CTX=8192` 启动）：5013 / 6853 token 的 prompt 都能从**中段**取回暗号（§9.9.4）。旧导出是 4096，对它传 8192 只会被静默降级；**反向也夹**——32768 的导出传 8192 仍挑 32768（§4.3） |
...
| **多用户接入（边缘服务器）** | ✅ **已实现**：身份显式化（`conversation_id`/`X-Conversation-Id`/`user`）+ **只排队不抢占** + `POST /v1/conversations/close`（客户端主动交还）+ 空闲回收 `IDLE_TTL` + `GET /v1/pool` 可观测；`NSESSION` 以内谁问谁拿，超过排队、超 `QUEUE_TIMEOUT` 返 503（§9.8）。**2026-09-16 起板上真后端已随整套件跑到**：身份 / 排队 / `close` / `/v1/pool`（§9.10.8）。**2026-09-18 补第二层回收**：`--contend-idle`（默认 15s）——只有"一个空闲会话都找不到"时才降下来，且只收最闲的那一段；排队的上界因此从 `IDLE_TTL`（300s）变成它，等待也变成可见字段（§9.14） |
...
模型上下文        8192（板上那份导出；**启动必须显式传 CTX=8192**，默认 4096 会静默砍半）
超长请求          >1 MiB 载荷 → 400「exceeds the frame limit」(0.1s)，会话一个不少；
                  改动前同一条 = 503 BrokenPipe + 后端退出 + 四个会话全 dead（§9.11）
...
会话排队          四段对话静默 70–90s 时第 5 段：改动前 226.0s（等满 IDLE_TTL）→ 改动后
                  15.4s（静默 0.5s 时敲）/ 0s（静默 >15s 时敲）。--contend-idle 默认 15，
                  只在"找不到空闲会话"时发作，且只收最闲的一段（§9.14）
```

---

*文档版本：v1.26（2026-09-20）—— **底部资源看板：七块卡片等高齐底、「服务」两列、宽度不够时整段折**（§9.21）。用户给的是结构性要求：① 七块卡片包进一个统一容器、`align-items:stretch`，**左侧 RK3588 的最底边必须与右侧 RK1828 的最底边在同一水平线上**（"解决目前视觉上高低不平的问题"）；②「服务」从单列改成 `grid-template-columns:1fr 1fr`；③ 统一卡片内外间距、内容少的卡片内部贴顶、"避免产生异常的视觉留白"。**先量再说**：拿板上真 `/v1/system` 喂进页面、无头 Chrome 量 `getBoundingClientRect()`，改前 1600×900 下左三块底边 771、右四卡 723——**差 48px**。**等高是三步叠出来的、缺一步都不齐**：① 外层 `.swrap` 的 `align-items` 用默认 `stretch`（原 `start`）；② 子网格 `.sgrid` 的 `align-content` **也要 `stretch`**——**最容易漏**，单行网格在 `start` 下只占内容高，外层 stretch 落不到卡片上；③ `.sblock` 默认 `align-self:stretch` 拉满整行，再 `display:flex`+`justify-content:flex-start` 让内容**贴顶**（齐平的是底边不是里面的行，内容少的热区那块空白全留底部）。列数**写死**（左 3 右 4、外层 `3fr:4fr`）换掉 `auto-fit`：它会在 1500px 以下自己折行（1366 → 右栏 3+1、1280 → 左栏 2+1），一折行两组高度立刻对不上、整块还更高（245→293px）；3:4 正好让七张卡**一样宽**。**「服务」两列的媒体查询两头都要有界**（`max-width:1500px` **且** `min-width:1181px`）：1500 以下每格只剩 ~65px、"数据 0.0s 前"折成两行、五个格子共六行——比一列五行还高（1280 上 210 vs 192）；1180 以下又退回上下两组、卡片变回 360px 宽、两列放得下，单列反而白高 48px（217 vs 235）。**宽度不够时的两种"折"**：温度那一行是七块里唯一长度不设上界的，`.m` 是 `nowrap` ⇒ 卡片一窄到 207px **文字溢出边框、压在隔壁卡上**（1600/1366/1280 实测都是这个症状），改成会折的 `inline-flex` + 每格不折的 `.ti`，且必须 `flex:1 1 auto;min-width:0`（flex 项默认 `min-width:auto` 会硬撑到内容宽、永远不折）；⚠️ **分隔符 `·` 必须是真文本**（写在每格开头），第一版挂在 CSS `::after` 上被检查判红——生成的内容复制不出来、读屏软件时常也不念（"npu 81° · bigcore0 51°"复制下来成了"npu 81°bigcore0 51°"），格间空格用**不折空格**；"内存"那一行则会在自己内部断成四截（1280 上 74px 高），给 `.srow` 加 `flex-wrap:wrap`、尾注标 `.m` 成为整体整段挪行。**顺带修掉一处静默截断**：`.syssec` 的 `flex-shrink` 从 1 改 0——它和 `main` 都是 1 时矮屏上多出来的高度**按基准尺寸分摊**，它被压到比 `max-height` 还小（1366×768 下 249px 内容只分到 185px），底部那句"NPU 是卡级忙时占比"的说明被切掉，**而那句正是防止两个百分数被当成硬件读数读走的**；现在它的高度只有"按内容"或"卡在 `max-height`"两种，要挤全由 `main` 让。**实测**：1920×1080 七块 255×117 一行、底边全 929、资源块占 **21%** 且不滚；1600 210×138 / 底边 762 / 28%；1366 176×165 / 640 / 34%；1280 164×165 / 661 / 34%；改前同一把尺子 1600 差 48px、1366 占 **49%**、1280 占 **54%**。**15 个布局变异体逐个试过、全部被抓**；⚠️ "分隔符必须是文本"这条**只有读页面真跑出来的文本才查得到**（桩 DOM 里 `::after` 不生成文本），且尾注那条断言**必须指到具体那一段**（带字节数/`title`）——泛泛地找 `class="m dim2"` 时页面上三处尾注改坏一处、另两处照样命中，第一版就是空转的、被变异体抓出来。**上板**：换演示页不用重启网关（`_static_demo` 现读磁盘），只传一个文件；`/demo` 与工作区逐字节相同、`page_check.js` 对真板卡 `===== 全部通过 =====`、1920 与 1366 两张截图确认等高齐底。⚠️ 对着真板卡跑检查前**先看 `/v1/pool`**：这次开工时池子里躺着一个上次遗留的 `id:web-baseline` 租约占着 session 0、状态 `busy`（`idle_s` 为 `null`，**回收器只收 idle 的、它永远不会自己好**），第一遍因此报"2 路没有落在 2 个不同会话上"*

*文档版本：v1.25（2026-09-20）—— **换端口（8080 → 18280）+ 收紧 CORS，以及"整页一屏"**（§9.20）。用户两条要求：① "不用 8080 端口，换成更安全的端口，注意信息安全"；② "对话框下面有很大的空余内容……资源展示放在网页底部约占整个网页的 1/4 到 1/5，将空白填满，可以适当将对话框拉长"。**①先说清楚换端口不是安全控制**：`HOST` 默认就是 `0.0.0.0`，同一局域网本来就都能连（§9.7 为 Agent 定的），扫一遍就知道服务在哪——**真正该修的是 CORS**。**②原实现的 `Access-Control-Allow-Origin: *` 配一个不要凭据的 API**，效果是"局域网里任何人打开的任意网页"都能在访客不知情的情况下驱动这块板卡（读 `/v1/system`、发起生成、拿 `/v1/pool` 的 key 把**别人的**对话 `close` 掉）；而演示页本来就是同源（`GET /demo`），那个 `*` 对正经用法一点用都没有。改法：`CORS_HEADERS` 常量换成纯函数 **`cors_headers(allowed, origin)`，默认参数下永远返回空**（这是整条防线唯一重要的性质），`--cors-origin`/`CORS_ORIGIN` 显式放行**一个**来源且**精确匹配**（不做子串——子串匹配是这类白名单最经典的写法错误），没配置来源时预检**只回空 204**。四个发响应的地方（`_json`/OPTIONS/`GET /demo`/SSE）各写各的，所以自检**真起 HTTP 服务、真读响应头**打一遍线路（纯函数对说明不了"每处都用了它、且回的是同一个答案"），变异体（还原成永远发 `*`）⇒ **16 FAIL**。端口同步到 `--port` 默认值、`start_gateway.sh`、全部 demo/验收脚本；**改端口不改帧协议**，后端二进制不用重编，但换网关仍必须成对换。⚠️ 改端口**不是单向的**：同事的 SSH 隧道、cc-switch 的 Base URL、书签都得跟着改。**②整页一屏**：`html,body{height:100%}` + `body` 一列 flex，标题/容量条/汇总条 `flex:none`，**对话区 `flex:1 1 auto;min-height:0` 吃掉剩余高度**（用 `100%` 不用 `100vh`——移动端地址栏会让 `100vh` 稳定地缺一条），面板不再写死高度，两列 `grid-auto-rows:minmax(0,1fr)` 平分；资源块从上下两块（376px，1080p 上 37%）**改成左右两栏**（`.swrap`，左三块右四卡）⇒ 高度由"相加"变 `max`，**实测 249px**：1080p **25%**、1440p **17%**——**一块内容没少也没挤紧**，只是不再白占一整行。**这类改动只能靠读源码验**（桩 DOM 没有排版引擎，空白它看不见），`page_check.js` 第 ⑩ 节因此是个**源码级 CSS 检查器**（剥注释 → 花括号配对扫出 `{选择器, 声明体, @media}` → `decl()` 取最后声明），判"会让空白回来的那几条"：`main` 有 `flex-grow≥1` **且** `min-height:0`（少这一条整页还是会被撑出屏幕）、**任何选择器都不许用 vh 写死高度**（就是原来那个坑）、资源块封顶 1/5~1/3 屏、`.swrap`/`.sg-host`/`.sg-cards` 三个类名 **JS 与 CSS 两边都在**（靠字符串联系，单方面改名页面照样跑、功能检查全过，只是悄悄长回 376px）。⚠️ **解析前必须先剥注释**——注释里恰好写着 `calc(50vh - 60px)` 这种"以前的写法"，变异体专门验了"只把 vh 写进注释不许红"。**测量**：无头 Chrome 注入测量脚本，六种窗口尺寸下 `body.scrollHeight == documentElement.scrollHeight == innerHeight`，另用 `--screenshot` 看过实物。**4 个布局变异体全被抓**。本轮**板上已重启到 18280 并跑通**（见 §9.20.7）*

*文档版本：v1.24（2026-09-20）—— **资源那一块挪到页脚，并把它的开销压到"与看的人数无关"**（§9.19）。用户两点要求：放到网页底部、别太紧凑；**尽量减少这个展示功能对硬件资源的占用**。**挪**：`#sys` 从标题下方搬到对话区（`main#grid`）之后、`</body>` 之前，仍常驻可见（"此刻板卡在干什么"任何时候都得看得见，只是不再抢在回答前面）。**排版**：换成两张网格——上面 `RK3588 / RK3588 热区 / 服务` 三块，下面**一张卡一块**（排成一行时四张卡看起来是一条连续的数据线，数不出张数），块间 10px、块内行距 7px、条子撑满行宽。**省**：一次快照 = 网关读它自己的 `/proc`（≈18 次打开）**+ 一次 STAT 往返**，而出这些钱的正是**跑着四路推理的那台机器**——"看的人越多、被看的越慢"是资源展示特有的反噬（`STAT_MIN_INTERVAL` 只节流了要帧那一步，`/proc` 那半是每请求都读）。三条改法：① **页侧两个档位**（生成中 2s / 待机 6s，切后台停轮询照旧）**+ 新一轮开始立刻补一拍**（不然待机档最长 6s 才看到卡忙起来）；② **同一份 `ts` 连 DOM 都不动**（`innerHTML` 重写 = 整块重新排版，而数字一个没变）——⚠️ 判断里必须带 `ts != null`，没有 `ts` 的应答会让 `undefined === undefined` 成立、这一页**从此不再刷新**（冻在打开那一帧，屏幕上看着还挺正常）；③ **网关侧 `SYS_CACHE_S = 1.0` 快照缓存**（一秒内不管几个人问只采一次，采的时候持锁，后来的人拿现成的）——它把开销从"与人数成正比"变成"每分钟最多 60 次"，**同时是②的前提**：没有缓存，同一秒里的两次轮询必然是两个不同的 `ts`，页侧那一半等于不存在；时间戳取**采完**的时刻（采本身可能要等 STAT 一秒，按开始时刻算它一出锁就过期）。外加**热区表只问一次**（有哪些区、叫什么开机就定，板卡上 7 个区 = 每拍 1 次 `listdir` + 7 次读 `type` ⇒ 只缓存表，每拍只剩读温度 7 次；只缓存**列成功**的结果）。**验证**：网关自检 **130 PASS / 0 FAIL**（新增 6 条钉缓存，含"**过期必须重采**"——只测"1 秒内不重采"的话，一个**永不过期**的写法照样全绿，而它在板上就是资源区冻住不动）；`page_check.js` 对桩网关 `sessions=2 @32768` 与 `sessions=4 @8192` 全过（新增 7 条）；本地对 8099 桩网关三次连问同一个 `ts`、隔 1.3s 再问换了新的。两个陷阱是同一个形状——**这功能坏了的样子是"不动"，而"不动"也正是它正常时的样子**（待机时温度内存本来就不怎么变），所以判据只能是"给它一个必须变的条件，看它变没变"。**9 个变异体全被抓**（页侧 6：`send()` 不调 `sysWake`／待机档写死／忘掉 `ts` 判空／永远不重画／卡那一行不画／位置判据反过来；网关侧 3：缓存永不过期／根本不缓存／采完不盖时间戳）——第一条尤其重要：前几节只证明 `sysWake` 这个函数有用，**证不了它被接在生成那条路上**，所以那条断言走的是真的 `send()`。**顺带修掉检查脚本自己的一处尾巴**：最后一节真发了轮请求，而收尾直接 `process.exit` ⇒ 租约留在池子里（板上 IDLE_TTL 会自己好，本地桩 `--idle-ttl 0` 是永久占着，同一个桩连跑第二遍就在"增删窗口"那一节红、看着像页面坏了）；现在走 `removePanel` 交还并**等它落地**（交还是异步发的），连跑两遍全绿、池子干净*

*文档版本：v1.23（2026-09-20）—— **资源条上板实测：两个只有真板卡能暴露的错**（§9.18.6）。**错一**：页面上每张卡写着「内存 19.0 MB」，而同一时刻 `rknn-smi` 报 93%——`rknn3_dev_mem_info` 里 `sys_total/sys_free` 是**本机侧的小块内存**（实测 19.0/18.7 MB，驱动与传输缓冲），**整卡内存是各 node 之和**（8 × ~634 MB ≈ **5.06 GB/卡**）；这一句在 `--probe` 的老注释里就写着，取字段时没照做。改完对账：2 路 @32K ⇒ 0~2 号卡 **92.4%**、3 号卡 **95.4%**、`node_min_free` 33.4 MB ×3 + **13.25 MB**，与 §9.17 记的「smi 93/95%」逐项吻合。**本机为什么测不出**：主机桩对设备内存一律回零 ⇒ `Σ node` 与 `sys_*` 都是 0，取哪个完全等效 ⇒ 桩改为**填一张确定的假表**（`sys_*` 直接用板上量到的两个 19 MB、node 数取 3 ≠ 夹具的 2 段），新增变异体 `stat_use_sys`（改回取 `sys_*`）必须让 t12 变红。**错二**：`/v1/system` 的 `busy_pct` 每隔几次从 `0.0` 变 `None`，页面在数字和 `—` 之间闪。根因是利用率=«两次 STAT **答复**之差 ÷ 墙钟»，而 `STAT_WAIT=0.35s` 太紧、板上**偶发**答复晚于它 ⇒ 超时的调用拿到上一份 doc（`now_us` 没动 ⇒ Δt=0）；更隐蔽的是那次采样还**无条件推进了窗口**，于是下一次即使拿到新 doc，Δ 也只剩后半段。**两处一起修**：`STAT_WAIT` 放到 1.0s（这条路径只服务 `/v1/system`，不在聊天请求路上）；同一份 doc 连着采两次时**既不推进窗口、也把上一次算出的百分比原样给回去**（同一份数据没变旧，`—` 留给"真的没有值"）——顺带修掉"多人同时看页面时晚到的那个人拿到 `—`"。**真机确认**：换上新网关后连采 8 次 `busy_pct` **全是 `0.0`、一次 `None` 都没有**（修前这一组里夹着 `None`），停手 3 秒回落到 `0.0`；两轮对话后**四张卡 `run_calls` 都是 40**（含 stage0，即 §9.18.4 那处两调用点修复在真机验证）；主机侧工作区版全绿、`HEAD` 版与 **10 个**变异体全在该失败处失败、网关自检 **124 PASS / 0 FAIL**、`page_check.js` 对桩后端与**真板卡**都全过*

*文档版本：v1.22（2026-09-20）—— **演示页资源条：把"板卡还剩多少、谁在忙"搬到屏幕上**（§9.18）。用户要求"把 RK1828、RK3588 的内存，NPU、CPU、温度等动态展示在网页端，展示效果自己优化"。**三类数据三条来路**：RK3588 的 CPU/内存/负载/开机时长由网关直接读 `/proc/*`（后端死了照常有）、温度读 7 个 thermal zone（**全部是 RK3588 自己的，含它那个 `npu-thermal`**）；而 **RK1828 那半边只有持设备者拿得到**——模型跑着的时候 `rknn-smi` 直接 `Failed to initialize rknnsmi`（设备同一时刻只认一个持有者），所以新增 **`STAT` 帧**（stdin 一行 `STAT` → `STAT <len>` + JSON）：**就地答复、不进调度队列**（否则会被前面那条长 decode 挡在后面，而那正是"想看它在忙什么"的时候），只读原子量、零设备访问；内存快照由**推理之后在卡锁内**顺手采（`rknn3_query` 与 `session_run` 走同一条到设备的通道，SDK 没承诺并发安全），节流 2s。**"忙时占比"是算出来的**（累计 `session_run` 耗时 ÷ 墙钟，**SDK 没有利用率查询接口**）；**卡温拿不到**（RK1828 是 PCIe 设备 `rkep`，无 hwmon、无 thermal_zone）。**口径四条**（都写进了页面常驻说明）：忙时用**单调钟**（NTP 校表会让比值荒谬）、`mem_at_us == 0` = "一次都没采到"而非"采过但很旧"（拿 0 算年龄会显示"53 年没更新"）、**缺值画 `—` 绝不画 `0%`**、查询失败**保留旧值且不更新时刻**。⚠️ **加一种帧 = 后端与网关必须成对换**（未知 tag 不消费载荷 ⇒ 整流错位，ERR 帧那次之后的第二次）；⚠️ **`session_run` 有两个调用点**（`run_stage_worker` 与 `run_pipeline_once` 里就地跑的 stage0），第一版只记了前者 ⇒ 第 0 张卡永远"空着"，是主机 t12 抓到的（桩不喂 batch，夹具里只有漏掉的那一半会执行），修法是把三行收成一个 `record_stage_run_stat`。**验证（当时只在本机）**：网关自检新增 `selftest_system`（假 `/proc`+假 `/sys` 夹具，含"只有 iowait 变化时 0%"、`MemAvailable` 不是 `MemFree`、坏温度项跳过）、9 个网关变异体 + 12 个页面变异体 + 3 个 t12 变异体全被抓、`page_check.js` 对桩后端全过；**板上当轮没跑**（随后补上，见 v1.23/§9.18.6）*

*文档版本：v1.21（2026-09-20）—— **32K 上下文：会话数由那份导出决定，页面窗口数不能再写死**（§9.17）。用户要求"32K 不强求 4 session，你测出最多能接受几个 session，然后网页端也要能按上下文限制增删窗口"。**先量**：32768 导出每路 KV **50.8 MB/node = 406.7 MB/卡**（建第 2 个会话时最紧 node 62.7 → 12.6 MB），卡上 5071 − 权重/internal 3993 ⇒ 可给 KV 的 ≈ 1078 MB/卡 ⇒ **每卡只够 2 路**（1 路 smi 84/88%、2 路 93/95%、第 3 路要再挤 50.8 而只剩 12.6）；单价 ≈ **12.4 KB/token/卡**，一条线性式同时对上 4096→5、8192→5、**32768→2** 三个实测点。**顺带纠了两处口径**：① §4.1/§10 那句"容量与上下文长度**完全解耦**"说的是"**这一路实际用掉**多少 token 不影响占用"（KV 按满上下文预分配），**不**是"上下文长度不影响容量"——容量 = **上下文 × 路数**；这句被引用两轮都没带限定词，最后落到"32768 导出配 4 路"（1626.8 MB/卡，超额 51%）；② §4.4 的"降 ctx 重导不能提高并发"**只在 KV 不是瓶颈时成立**，32K 下 KV 就是瓶颈、降 ctx 正是唯一正解。**并把撞穿的现场记全**：`rknn3_session_init` 分配失败**不报错**，卡在 `wait_event(0x100000): timed out` → 端点从此不应答 → 之后一律 `ERROR_PIPE` → `systemctl restart rknn3` 反而带走四个 proxy（退出码 234）⇒ **只能重启板卡**；最坑的是 `CTX=4096` 与 `NSESSION=4` **各自都合理、组合才致命**。**三道防线**：① `start_gateway.sh` 启动闸（从 rope 外置张量大小反推上下文——每位置 256 B + 432 B 头，8192→2097584 B / 32768→8389040 B——算不过就**拒绝启动**，`FORCE_SESSIONS=1` 硬上；读不到**不拦**，且"读不到"不能当"安全"）；② `/health` 新增 `ctx_size`；③ 页面窗口数 = `min(4, sessions)`。**页面**（`demo_4chat.html`）：常驻容量条（`上下文 32,768 · 后端会话 2 · 窗口 2/2`）、＋/× 增删、**身份 `web-<n>` 单调不复用**（复用等于把"新对话"伪装成"老对话的续聊"，而 KV 前缀判据按身份粘会话，**只错不慢**）、删窗口**立刻 `closeConv` 交还**、显示编号 `面板 N` 与身份解耦；**没让用户直接挑 N**——板上这份导出物理上就没有 4 路，能挑一个后端没起的数是空头承诺（点下去就是 §9.17.3），"少用"只能靠删。顺手修掉 `#addwin` **有按钮没监听器**的真 bug；`addPanel` 自己的守卫才是防线，按钮 `disabled` 只是提示（`/health` 是异步的，它回来之前窗口已按默认值建好）。**验证**：`page_check.js` 新增"建窗与容量条"+"增删窗口"两段、夹具 3 → 5 组（多两组 **2 路**形态），**五个变异体全部 FAIL**（写死 4 / 身份复用 / 去掉 `closeConv` / 去掉容量守卫 / 到顶不 `disabled`）；本机桩后端三种配置全绿（4 路、6 路 @8192、**2 路 @32768**）；**真板卡 `CTX=32768 NSESSION=2` 全绿**（启动闸打"最多 2 路会话"、后端 `max_ctx_len=32768` + `2 conversations ready`、端到端两路 8.72/8.79 tok/s）。⚠️ **板上那套回归套件在 `NSESSION=2` 下报 3 条 FAIL，不是回归**：`git diff` 证明本轮网关改动只有 print 与 `/health` 加字段，且 `toolcall_check.py` 单独在**空闲池**上 PASS（热=32/冷=630，套件里是 630/630——套件按 4 路写、会占满池子，2 路下前缀判据必然落空）。**未验证**：32K 下"4 个窗口"的形态板上见不到（物理只有 2 路），只在桩后端（6 路）验过；新容量条与 ＋/× 的排版只在无头浏览器看过，没在真浏览器点过窄窗口/触摸屏*

*文档版本：v1.20（2026-09-18）—— **演示页"聚合 tok/s"的口径：两个"平均"不能混用**（§9.16）。用户报的一次真事：四路各自 9.83/10.07/11.80/9.72 tok/s（**每路都是全速**，单路基线约 10.6），横幅却写"合计 2095 tok / 墙钟 135.7s → 聚合 15.43 tok/s"，跟单路一个量级。**不是算错，是口径**：那 135.7s 里只有前约 18 秒是四路在跑（面板 3 生成 1602 token 跑了 135.7s，另三路约 16s 收工），后约 120 秒**只有一路**在跑，`Σtok/墙钟` 把"一路的吞吐"摊进了"四路并发"的分母 ⇒ **各路速率和 41.42 tok/s**（四路同时跑的总产出，**和单路基线同一个口径**）vs **墙钟摊薄 15.43**（平均只有 **1.37/4** 路在跑），**差 2.7 倍**。改法（`demo_4chat.html` 的纯函数 `summarize()`）：横幅主数改用**各路速率和**，**伸缩比只许用它**（同一轮用摊薄值会得到 1.45×，与 3.89× 差到会改结论）；摊薄值照常印但只在四路长度齐时才有意义，`Σel/墙钟 < n−0.5` 时横幅自己标"四路长短不齐，平均只有 1.37/4 路在跑……别当并发性能看"；排队警告移到**整句最后**（它撤的是刚印出来的伸缩比）。顺带修了一个**检查自己的盲区**：`page_check.js` 原先自己 `panels.map(send)` 重发四路，页面里 `summarize + setSum` 那两句**压根不执行** ⇒ 汇总行怎么算的**没有任何覆盖**（这次的口径问题能活到现在，这是原因之一）；现在把「②」的处理器提成 `runAll()`，检查直接调它、用 `lastRound()` 取回结果，并核对页面上**真写出来的那一句**（只比"各路速率和"这一项——两边墙钟分别来自 `performance.now()` 与 `Date.now()`，拿整句比等于在测时钟抖动）。另加一组**不联网的纯算术 fixture**：长短不齐 / 四路等长 / 某路排队三种，用的是**用户那次真实运行的数**；三个变异体（伸缩比改用摊薄值、稀释警告永不触发、把摊薄当主数印）**全部 FAIL，且都在联网那部分之前就 exit**。验证：本机桩后端 `page_check.js` **全部通过**（含"页面写的 = 独立算的"），无头浏览器截图看过最长形态在 1280 宽下**换两行**（不联网，直接喂真实数字），文件换到板上后 md5 一致、旧件留 `serve/bak_20260918_sum/`。⚠️ **真板卡这次没跑**：下发时板上网关没在起（`:8080` 拒连、NPU 代理在而网关进程不在），**没有为这一条去重启一个要 240s 加载模型的服务**；fixture 里的四个数是**从用户屏幕抄的**，钉住的是"公式对这几个数给出什么"，不是"板上一定会出现这几个数"；阈值 `n−0.5` 只在桩上试过正常抖动*

*文档版本：v1.19（2026-09-18）—— **演示页的 UI/UX 改版**（§9.15）。用户点名要的排版改动：徽标化的运行状态、右侧控件对齐、按钮三级、数字框、**面板头右侧的 TPS/Latency 小字**、面板内边距、**多行自适应输入框**、发送键与输入框对齐。**三处没照原话做，理由都不是"我懒得做"**：① **不用 Tailwind CDN** —— 这一页是板卡**离线**发出来的（`_static_demo`），演示现场未必有外网，引 CDN 等于给"打不开页面"埋一个只在断网时才发作的雷；要的是那套层级（primary/secondary/danger、`flex items-center gap-4`）不是那个 CDN，所以按同样意图写成页内 CSS 变量。② **指示灯用 CSS 不用 emoji** —— 🟢/🟡 在 Windows/安卓/iOS 上长得不一样、投影出来还可能是方框，而这块牌子恰恰要隔着投影仪一眼看出板卡在不在跑。③ **面板头那格与底部统计行分工**：上面只放**首字延迟 + tok/s**（底部那行**看不见 TTFT**，而"要等多久才出第一个字"是给人看并发最直观的一个数），下面是记账（token 数、排队拆解、会话、KV）——不是同一件事说两遍。顺手堵了两个"屏幕上会骗人"的地方：**徽标必须字和色一起换**（同一个灰灯下"生成中"和"待机"没区别），**新一轮开始时那格必须清空**（挂着上一轮的首字延迟，在最该判断"到底开始了没有"的那一帧给旧数字，比空着更坏——`page_check.js` 新增一条断言守它，另一条守"性能小字还在不在"：`data-role` 拼错、元素被删、`setPerf` 忘了调，这三种坏法页面都照跑照出答案、只是那格永远空着）。回车分流也改了：**Enter 发送、Shift+Enter 换行**（单行框里回车只有"发送"一个意思，换成 textarea 后默认是换行，不拦就是"按回车没反应"）。验证分三层，各有边界：桩 DOM 的 `page_check.js`（**本机桩后端全绿 + 真板卡全绿**，板上四路 `web-1..4` 落四个会话、TTFT **1.0–1.7s**、各路 ~10 tok/s、聚合 35.73、关页面与「清空」都把会话还干净）+ **无头浏览器截图**（真 CSS 排版，本机桩与真板卡各一张，真机那张抓到的正是**运行中**一帧：四格蓝框、琥珀徽标、性能小字空着、输入框已缩回一行）+ 人眼。⚠️ **截图那层没有断言**，"我看过了"不等于"以后不会坏"，真正守得住这次改动的只有那两条性能小字断言，配色与间距仍只能靠人眼；也**没在窄窗口/触摸屏上试过**（`.controls` 排不下会换行，1024 宽以下没量过）。页面是**现读现发**的 ⇒ **只换文件、不重启网关**（省掉 240s 模型重载），两个文件下发前后 md5 逐一核对一致，旧件留 `serve/bak_20260918_ui/`。另记一条工序坑：临时探针里那段多行文本，Python heredoc 的 `\\n` 没活下来变成真换行 → JS 字符串断行 → **整段探针语法错、一声不响地不执行**，截图看起来只是"页面没变化"；`--dump-dom | grep` 也骗了我一次（grep 命中的是**脚本源码里的字符串**，不是渲染结果），判据要看**只有运行时才会出现的属性**（`class="badge busy"`、`data-probe-err`）*

*文档版本：v1.18（2026-09-18）—— **NPU 空着却在排队：会话池加一层「有人在等就收最闲的那一段」**（§9.14）。用户报的是"明明 NPU 现在没人使用，我输入问题后还是在等待"，网页那格写着 `158 tok · 100.9s · 1.57 tok/s`。**先把"慢"拆开**：四路并发的 TTFT 1.09–1.75s、中位间隔 0.084–0.092s，**解码侧一点没坏**；坏的是另一边——pool 快照显示排队那段时间**四个会话全是 `idle`**，NPU 上一个请求都没跑。根因是会话池只有一条回收阈值 `IDLE_TTL`（300s），**且 `_reap_idle_locked` 只在 `acquire` 里被调用**（按需回收的刻意设计：不静默重算任何一段对话的 KV），两条合起来就成了"N 段对话把 N 个会话绑着不用时，第 N+1 段要等到某一段静默满 300s"——**等的是阈值，不是算力**；板上实测第 5 段**排队 226.0s**，它拿到会话的瞬间正是 `web-1` 跨过 300s 的那一刻。而页面上那个 `1.57 tok/s` 是**端到端**的数（SSE 响应头在拿到租约之后才 flush），排队被算进了分母，屏幕上没有任何字段能把它和"模型慢"分开。修法是**两层阈值**：新增 `--contend-idle`（默认 15s），**只有**一个空闲会话都找不到、这一轮非等不可时才降下来，且**只收最闲的那一段**（还有空闲会话时一次都不发作，常规路径逐字节不变；`idle_ttl=0` 仍是总开关——回归套件正是靠它证明自己真在主动 `close`）。板上 A/B（同一块板、同一份模型，只换网关进程）：四段对话静默 70–90s 时第 5 段从 **排队 226.0s → 15.4s**（在静默 0.5s 时敲，`wait=15.0`）/ **0s**（在静默 >15s 时敲，`wait=0.0`、TTFT 1.17s，这是常见形态），`reaped_total` 一次只 +1，静默 41.8s 的两段**仍不被收**。同时把"等"变成可见的：`X-KV-Reuse` 加 `wait=`、日志 `lease:` 行排过队就打 `排队 X.Xs`、`/v1/pool` 报 `contend_idle_s`、演示页行内显示等待态并把统计行拆成"排队 / 实际推理"、有人排过队时汇总栏标"伸缩比不可引用"。验证：网关自检 / `serve_http_test.py` / `page_check.js`（桩后端）全绿；板上 `run_all_board_tests.sh` **8 段 rc=0 / FAIL 0 / 非零 rc 0 / traceback 0**（10.92 / 20.27(1.86×) / 36.80(3.37×) tok/s），`page_check.js` 对真板卡 + 真模型全绿（36.13 tok/s、四路落四个会话、关页面与清空都把会话还干净）。⚠️ 改的是**网关进程**，必须重启（模型重载 ~240s）；旧件留 `serve/bak_20260918_contend/`*
