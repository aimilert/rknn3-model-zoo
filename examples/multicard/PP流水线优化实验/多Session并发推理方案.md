...
9. **M5（服务化）已实现并实测**：`main.cc --serve` 提供带 `rid` 的**帧协议**（走独立 fd，避开 stdout 上的日志污染），板端 `python3` 网关（`examples/multicard/serve/`）对外提供 **OpenAI 兼容** `/v1/chat/completions`（SSE + 非流式）、`/v1/models`、`/health`，并做会话粘性 + 跨轮 KV 复用。同一份脚本量 HTTP 层并发伸缩：**10.97 / 20.28 / 37.24 tok/s = 3.40×**，对照官方 `rkllm3-server` 同板同条件 **10.63 / 10.98 / 10.98（1.03×）**——**收益来自我们自己的并发执行器，不是 HTTP 层**；HTTP 层要证明的只是"没把它吃掉"。踩到的三个坑**全部只影响速度、答案完全正确**（详见 §9.7）。
...
- 脚本里的板卡/云路径**全部参数化**（`INSTALL_DIR` / `MODEL_DIR` / `GATEWAY_DIR` / `LOG` …，
  ...
  仓库副本 **md5 相同**（`e09ac2b941de767443c0d1d635bd305b`），两份副本不再有漂移。
...
| **工作量** | P0/P1/P2/P3(交互式)/R11/M5 已完成；**M5 的交付件已全部入库**（`cpp/main.cc` 的帧协议 + `examples/multicard/serve/`） |
...
| **下一步** | ① 定性 SDK 内部的 TSan 报告（需原厂/SDK 源码）——**优先那 4 条写侧在我方 `input_callback` 的**；② ~~ASan 跑一轮~~ → **已跑（§9.3）**；要把 R4 彻底关闭需在 ASan 下跑满 30 分钟 soak；③ ~~R11 卡锁~~ → **已验并补锁（§9.6）**；④ ~~HTTP 前端服务化（M5）~~ → **已完成（§9.7）**；⑤ **接 workflow agent 前补测 `tool` 角色渲染**（§9.7 未验证项）；⑥ ~~决定网关的入库位置~~ → **已定：移进仓库 `examples/multicard/serve/`（§9.7）**；⑦ 每卡卡级统计拆到会话维度 |
...
*文档版本：v1.6（2026-09-15）—— **M5 服务化**（§9.7 新增：帧协议 / 网关设计 / HTTP 层伸缩 3.40× vs 官方 1.03× / 三个"只慢不错"的坑 / 不可复现性 / 未验证项）；**网关与板端脚本已入库 `examples/multicard/serve/`**（原先只在 `rt_work/` 而它要下发到板卡，违反了 §8.1 红线——按"要落到板卡上的东西属于仓库"纠正，路径全部参数化，入库后重新下发并重跑全量套件）；联动 §0(第 9 条) / §5(M5 指针) / §6(M5) / §7(R12/R13) / §8.1(快照表 + 红线复盘) / §10*
