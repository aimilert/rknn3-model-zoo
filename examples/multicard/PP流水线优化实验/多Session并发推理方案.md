...
| **RK1828 每卡内存** | `RKNN3_QUERY_DEVICE_MEM_INFO`（`rknn3_dev_mem_info`），由**后端**采 | **设备同一时刻只认一个持有者**：模型跑着的时候 `rknn-smi` 直接 `Failed to initialize rknnsmi` |
...
**未验证**：① 板上真实数值（本轮**没有上板**——要上就得整套换后端+网关，模型重载 240s）；
② 卡温拿不到这件事只在 `rknn-smi`/驱动层面确认过，没在板上再抓一次；③ 资源条的**视觉排版**
只在无头浏览器里看过。
