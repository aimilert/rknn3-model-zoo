...
*文档版本：v1.7（2026-09-15）—— **修正 M5 的伸缩结论**：新增坑 4（会话租约算错 → 并发被偶发串行化；修复前同构建量到过 3.40× 也量到过 1.00×），因此 §0/§5/§6/§7(R13)/§9.7/§10 里的 **3.40× 全部改为修复后连量 4 轮的 3.33×（3.32~3.38）**，并写明"单轮数字不可作为结论"；补齐坑 4 的回归用例（修复前代码上复现失败）与租约落点这一诊断手段；顺带修掉 `run_all_board_tests.sh` 汇总块自引用 `grep` 的报错*
