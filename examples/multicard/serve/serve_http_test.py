...
def chat(messages, max_tokens=64, stream=False, **extra):
...
def main():
    ...
    print("== 流式 SSE ==")
    chunks, text = chat([{"role": "user", "content": "数到五。"}], max_tokens=32, stream=True)
    ...
    def kv_reuse_case(label, extra):
        ...
        d1 = chat([{"role": "user", "content": filler}], max_tokens=16, **extra)
        a1 = d1["choices"][0]["message"]["content"]
        hist = [{"role": "user", "content": filler},
                {"role": "assistant", "content": a1}]
        d2 = chat(hist + [{"role": "user", "content": "继续。"}], max_tokens=16, **extra)
    ...
    kv_reuse_case("开思考", {})
    kv_reuse_case("关思考", {"chat_template_kw": {"enable_thinking": False}})
    ...
    d4 = chat([{"role": "user", "content": "1+1=?"}], max_tokens=64,
    ...
    d6 = chat([{"role": "user", "content": "1+1=?"}], max_tokens=64)
