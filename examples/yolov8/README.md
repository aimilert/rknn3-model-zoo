...
**使用方式：**
- 不需要后处理插件 `libpostprocess_yolov8_rk182x.so`
- 模型后处理已内置，推理时自动执行
- 直接输出检测结果
...
### 模型下载链接
- yolov8n.onnx: `https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov8/yolov8n.onnx`
- yolov8n_rknn3.onnx : `https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov8/yolov8n_rknn3.onnx`
- yolov8s.onnx: `https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov8/yolov8s.onnx`
- yolov8s_rknn3.onnx: `https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov8/yolov8s_rknn3.onnx`

### ONNX 模型导出
完整的 onnx 导出方法可以从以下链接下载完整的 PyTorch 示例工程：
`https://ftrg.zbox.filez.com/v2/delivery/data/95f00b0fc900458ba134f8b180b3f7a1/examples/yolov8/yolov8-postprocess.tar.gz`
...
```bash
cd python
python convert.py ../model/yolov8n.onnx RK1820 i8
```
...
编译成功后会生成 `libpostprocess_yolov8_rk182x.so`
...
编译完成后，会在安装目录 `install/rk3588_linux_aarch64/rknn_yolov8_demo/`（需推送到板端的即此目录）下生成完整的运行包：
- `rknn_yolov8_demo`：单张图片推理程序
...
**通用命令格式：**
```bash
./rknn_yolov8_demo <model_path> <weight_path> <image_path> <core_mask> [postprocess_plugin_path]
```
...
./rknn_yolov8_demo model/yolov8n.rknn model/yolov8n.weight model/bus.jpg 0x1 lib/libpostprocess_yolov8_rk182x.so
```

**模式 B：不带_rknn3模型，主控端后处理（❌ 不推荐）**
```bash
# 不使用插件，后处理在主控端 CPU 执行
./rknn_yolov8_demo model/yolov8n.rknn model/yolov8n.weight model/bus.jpg 0x1
```

**模式 C：带_rknn3模型（⭐ 推荐多核）**
```bash
# 使用内置后处理的模型，不需要插件
./rknn_yolov8_demo model/yolov8n_rknn3.rknn model/yolov8n_rknn3.weight model/bus.jpg 0x1
```

**核心代码说明（模式 A）：**

在 `yolov8.cc` 的 `init_yolov8_model()` 函数中，通过传入 `postprocess_plugin_path` 参数来启用后处理插件：
...
**重要说明：**
- 后处理插件 `libpostprocess_yolov8_rk182x.so` 配合**不带_rknn3后缀**的模型使用
...
控制台输出示例：
```
person @ (109 236 226 535) 0.910
bus @ (99 136 555 439) 0.895
person @ (475 232 559 521) 0.879
person @ (212 241 286 508) 0.847
person @ (80 326 123 517) 0.592
```
...
```bash
# 推送整个安装包到板端
adb push install/rk3588_linux_aarch64/rknn_yolov8_demo /userdata/
...
./dataset_eval model/yolov8n.rknn model/yolov8n.weight 0x1 lib/libpostprocess_yolov8_rk182x.so
```

**模式 B：不带_rknn3模型，主控端后处理（不推荐）**
```bash
# 不使用插件，主控端后处理（数据传输开销大）
./dataset_eval model/yolov8n.rknn model/yolov8n.weight 0x1
```

**模式 C：带_rknn3模型（推荐多核）**
```bash
# 使用内置后处理的模型（与默认 core_num=1 转换匹配，使用单核 0x01 运行）
./dataset_eval model/yolov8n_rknn3.rknn model/yolov8n_rknn3.weight 0x01
```
...
```bash
# 从板端拉取结果文件
adb pull /userdata/rknn_yolov8_demo/results_rknn.json ./
```
...
后处理插件 `libpostprocess_yolov8_rk182x.so` 是 RKNN3 平台的自定义算子扩展机制，用于**模式 A**（不带_rknn3模型 + 后处理插件）。

**工作原理：**
- 配合**不带_rknn3后缀**的标准 YOLOv8 模型使用
...
├── yolov8_postprocess.c       # YOLOv8 后处理核心算法
...
编译成功后会生成 `libpostprocess_yolov8_rk182x.so` 动态库文件。
...
rknn3_load_model_from_path(ctx, "yolov8s.rknn", "yolov8s.weight");
...
// 步骤 4: 注册后处理插件（关键步骤）
const char* plugin_path = "libpostprocess_yolov8_rk182x.so";
...
**注意事项：**
1. ⚠️ **插件配合不带_rknn3后缀的模型使用**（如 `yolov8s.rknn`，而非 `yolov8s_rknn3.rknn`）
...
2. **模型与插件不匹配**：
   - ✅ 正确：`yolov8s.rknn` + `libpostprocess_yolov8_rk182x.so`
   - ❌ 错误：`yolov8s_rknn3.rknn` + 插件（带_rknn3模型不需要插件）
